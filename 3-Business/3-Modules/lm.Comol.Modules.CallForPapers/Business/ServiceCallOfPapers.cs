        ...
        #region "Call Presentation Methods"
            ...
            private List<long> GetCallsForEvaluationId(Boolean fromAllcommunities, Boolean forPortal, liteCommunity community, litePerson person, DisplayEvaluationStatus status)
            {
                return (from s in Manager.GetIQ<lm.Comol.Modules.CallForPapers.Domain.Evaluation.Evaluation>()
                        where s.Deleted == BaseStatusDeleted.None && s.Status != Domain.Evaluation.EvaluationStatus.EvaluatorReplacement && s.Status!= Domain.Evaluation.EvaluationStatus.Invalidated && (fromAllcommunities || (forPortal && s.Call.IsPortal) || (!forPortal && s.Community == community)) && s.Evaluator.Person == person
                         && (status == DisplayEvaluationStatus.Any || (status == DisplayEvaluationStatus.Evaluated && s.Evaluated == true) || (status == DisplayEvaluationStatus.ToEvaluate && s.Evaluated == false))
                    ...
                    switch (status) { 
                                    ...
                                    new dtoCall()
                                        {
                                            ...
                                            AllowSubmissionExtension = s.AllowSubmissionExtension
                                        }
                                    )).ToList();
                            break;
                        ...
                        case CallStatusForSubmitters.ToEvaluate:
                            ...
                            items =(from c in Manager.GetIQ<CallForPaper>()
                                where c.Deleted == BaseStatusDeleted.None && idCallsToEvaluate.Contains(c.Id)
                ...
                public CallForPaper SaveCallSettings(dtoCall dto, int idCommunity, Boolean validate)
                {
                    ...
                    try
                    {
                        ...
                        if (dto != null)
                        {
                            ...
                            callForPaper.AllowSubmissionExtension = dto.AllowSubmissionExtension;
            ...
            public Boolean isEvaluatorOfCallForPaper(long idCall, int idPerson)
            {
                ...
                try
                {
                    iResponse = (from e in Manager.GetIQ<lm.Comol.Modules.CallForPapers.Domain.Evaluation.Evaluation>()
                                 where e.Call.Id == idCall && e.Evaluator.Deleted == BaseStatusDeleted.None && e.Evaluator.Person !=null
