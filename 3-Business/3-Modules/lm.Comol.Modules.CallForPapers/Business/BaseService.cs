...
namespace lm.Comol.Modules.CallForPapers.Business
{
    public abstract class BaseService : iLinkedService
    {
                ...
                public Boolean IsCallAvailableByUser(long idCall,litePerson person)
                {
                    ...
                    if (person != null)
                    {
                        permission = (from pa in Manager.GetIQ<BaseForPaperPersonAssignment>()
                                      where pa.BaseForPaper.Id == idCall && pa.AssignedTo.Id == person.Id  && !pa.Deny && pa.Deleted == BaseStatusDeleted.None
                        ...
                        if (!permission) {
                            ...
                            {
                                permission = (call.ForSubscribedUsers && (person.TypeID != (int)UserTypeStandard.Guest && person.TypeID != (int)UserTypeStandard.PublicUser)) ||
                                              (from pa in Manager.GetIQ<BaseForPaperPersonTypeAssignment>()
                                              where pa.BaseForPaper.Id == idCall && pa.AssignedTo == person.TypeID && !pa.Deny && pa.Deleted == BaseStatusDeleted.None
                                              select pa.BaseForPaper.Id).Any();
                            }
        ...
        #region "Edit Cfp"
            ...
            public dtoBaseForPaper GetDtoBaseCall(long idCall)
            {
                ...
                try
                {
                    call = (from c in Manager.GetAll<BaseForPaper>(c=>c.Id == idCall)
                     ...
                     {
                         AllowSubmissionExtension = c.AllowSubmissionExtension,
                         Deleted = c.Deleted,
            ...
            public RequestForMembership SaveCallSettings(dtoRequest dto, int idCommunity, Boolean validate)
            {
                ...
                try
                {
                    ...
                    if (dto != null)
                    {
                        ...
                        call.AllowSubmissionExtension = dto.AllowSubmissionExtension;
            ...
            #region "Clone Call"
                ...
                private RequestForMembership CloneCall(RequestForMembership sourceCall, Int32 idCommunity, litePerson person, String prefix)
                {
                    ...
                    call.AllowSubmissionExtension = sourceCall.AllowSubmissionExtension;
                    ...
                    try
                    {
                        ...
                        { 
                            List<SubmitterType> submitters = (from s in Manager.GetIQ<UserSubmission>()
                                             where s.Call== attachment.Call && s.Owner==person && s.Deleted== BaseStatusDeleted.None
            ...
            #region "Editor"
                ...
                #region "Section"
                    ...
                    public Boolean SaveSections(long idCall, List<dtoCallSection<dtoCallField>> sections)
                    {
                        BaseForPaper call = Manager.Get<BaseForPaper>(idCall);
                        return SaveSections(call, sections);
                    }
                    public Boolean SaveSections(BaseForPaper call, List<dtoCallSection<dtoCallField>>sections){
                        Boolean result = false;
                        try
                        {
                            litePerson person = Manager.GetLitePerson(UC.CurrentUserID);
                    ...
                    public List<dtoCallSection<dtoCallField>> GetEditorSections(BaseForPaper call) {
                        ...
                        try
                        {
                            sections = (from s in Manager.GetIQ<FieldsSection>()
                                        ...
                                        select new dtoCallSection<dtoCallField>() { Id = s.Id, Name = s.Name, 
                                            Description = s.Description, DisplayOrder = s.DisplayOrder }).ToList();
                            ...
                            foreach (dtoCallSection<dtoCallField> section in sections) { 
                                ...
                                section.Fields.ForEach(f=>f.Submitters= (from s in Manager.GetIQ<FieldAssignment>()
                                                                         where s.Deleted== BaseStatusDeleted.None && s.SubmitterType!=null && s.Field !=null && s.Field.Id== f.Id
                ...
                #region "Field"
                    public FieldDefinition CloneField(litePerson person,FieldDefinition sField, FieldsSection dSection, Boolean rename)
                    {
                        FieldDefinition field = null;
                        if (sField != null && dSection != null) {
                            Int32 displayOrder = (!dSection.Fields.Any() || dSection.Fields.Count==0) ?  1 : dSection.Fields.Where(f=>f.Deleted== BaseStatusDeleted.None).Select(f=>f.DisplayOrder).Max() + 1;
                            field = new FieldDefinition();
                            switch (sField.Type) { 
                                case FieldType.CheckboxList:
                                case FieldType.RadioButtonList:
                    ...
                    private List<FieldOption> CloneOptions(litePerson person, FieldDefinition field, List<FieldOption> items)
                    {
                        ...
                        {
                            int displayNumber = 1;
                            foreach (FieldOption item in items)
                            {
                ...
                #region "FieldOptions"
                    public FieldOption AddOptionToField(long idField, List<dtoCallSection<dtoCallField>> sections, String name, Boolean isDefault, Boolean isFreeValue )
                    {
                        ...
                        if (field != null && field.Call !=null )
                            SaveSections(field.Call, sections);
        ...
        #region "Submission"
            ...
            #region "Submit"
                #region "1 - LoadItems"
                    ...
                    public List<dtoCallSection<dtoSubmissionValueField>> GetSubmissionFields(BaseForPaper call, long idSubmitter, long idSubmission, long idRevision)
                    {
                        SubmitterType submitter = Manager.Get<SubmitterType>(idSubmitter);
                        if (call != null && submitter != null )
                        {
                            try
                            {
                                ...
                                List<dtoSubmissionValueField> fields = (from f in Manager.GetIQ<FieldDefinition>()
                                                                        where f.Call == call && idFields.Contains(f.Id) && f.Deleted == BaseStatusDeleted.None 
                            ...
                            try
                            {
                                ...
                                if (call != null && call.Status != CallForPaperStatus.Draft)
                                {
                                    if (submission == null)
                                    {
                                        ...
                                        submission.isAnonymous = (submitter.TypeID == (int)UserTypeStandard.Guest) || (submitter.TypeID == (int)UserTypeStandard.PublicUser);
                ...
                #region "3 - Complete items"
                    public void UserCompleteSubmission(UserSubmission submission, Revision revision, DateTime initTime, int idUser, String BaseFilePath, List<dtoSubmissionValueField> fields, lm.Comol.Core.MailCommons.Domain.Configurations.SmtpServiceConfig smtpConfig, String websiteUrl, Dictionary<SubmissionTranslations, string> translations, String translationFileName)
                    {
                        litePerson person = Manager.GetLitePerson(idUser);
                        UserCompleteSubmission(submission, revision, initTime, person, BaseFilePath, fields, smtpConfig, websiteUrl,translations, translationFileName);
                    }
                    public void UserCompleteAnonymousSubmission(UserSubmission submission, Revision revision, DateTime initTime, String BaseFilePath, List<dtoSubmissionValueField> fields, lm.Comol.Core.MailCommons.Domain.Configurations.SmtpServiceConfig smtpConfig, String websiteUrl, Dictionary<SubmissionTranslations, string> translations, String translationFileName)
                    {
                        UserCompleteSubmission(submission, revision, initTime, submission.Owner, BaseFilePath, fields, smtpConfig,websiteUrl, translations, translationFileName);
                    }
                    private void UserCompleteSubmission(UserSubmission submission, Revision revision, DateTime initTime, litePerson owner, String BaseFilePath, List<dtoSubmissionValueField> fields, lm.Comol.Core.MailCommons.Domain.Configurations.SmtpServiceConfig smtpConfig, String websiteUrl, Dictionary<SubmissionTranslations, string> translations, String translationFileName)
                    {
                        if (!submission.Call.AllowLateSubmission(initTime, submission) || !AllowTimeRevision(revision, initTime))
                        {
                                ...
                                else
                                {
                                    switch (field.Type) { 
                                        ...
                                        case FieldType.RadioButtonList:
                                            ...
                                            if (fieldValue == null && field.Mandatory)
                                                items.Add(field.Id, FieldError.Mandatory);
                                            else if (fieldValue != null && !String.IsNullOrEmpty(fieldValue.Value))
                                            {
                                        ...
                                        default:
                                            ...
                                            if ((genericValue==null || String.IsNullOrEmpty(genericValue.Value)) && field.Mandatory)
                                                items.Add(field.Id, FieldError.Mandatory);
                                            break;
                                    }
                                }
                ...
                #region "5 - Mail Notification"
                    public void SubmissionMailNotification(UserSubmission submission, litePerson submitter, List<dtoSubmissionValueField> fields, lm.Comol.Core.MailCommons.Domain.Configurations.SmtpServiceConfig smtpConfig, String websiteUrl, Dictionary<SubmissionTranslations, string> translations)
                    {
                        if (submission.Call != null)
                        {
                            Int32 idUserLanguage = (submitter.TypeID == (int)UserTypeStandard.Guest || submitter.TypeID == (int)UserTypeStandard.PublicUser) ? 0 : submitter.LanguageID;
                            ...
                            List<dtoManagerTemplateMail> templates = (from t in Manager.GetIQ<ManagerTemplateMail>()
                                                                      ...
                                                                      select t).ToList().Select(t => new dtoManagerTemplateMail() { Id = t.Id, Body = t.Body, Deleted = t.Deleted, IdLanguage = t.IdLanguage, Subject = t.Subject, MailSettings = t.MailSettings, NotifyFields = t.NotifyFields, NotifyTo = t.NotifyTo }).ToList();

                            foreach (dtoManagerTemplateMail template in templates.Where(t => !String.IsNullOrEmpty(t.NotifyTo)).ToList())
                            {
                                lm.Comol.Core.Mail.MailService mailService = new lm.Comol.Core.Mail.MailService(smtpConfig, template.MailSettings);
                                lm.Comol.Core.Mail.dtoMailMessage message = new lm.Comol.Core.Mail.dtoMailMessage(AnalyzeContent(submission.Call, template.MailSettings.IsBodyHtml, template.Subject, submission, submitter, fields, websiteUrl, translations), AnalyzeContent(submission.Call, template.MailSettings.IsBodyHtml, template.Body, submission, submitter, fields, websiteUrl, translations));
                                message.FromUser = smtpConfig.GetSystemSender();
                                mailService.SendMail(idUserLanguage,dLanguage, message, template.NotifyTo, lm.Comol.Core.MailCommons.Domain.RecipientType.To);
                            }

                            dtoSubmitterTemplateMail submitterTemplate = GetSubmitterTemplateMail(submission, submitter);
                            if (submitterTemplate != null)
                            {
                                String mailField = (from dtoSubmissionValueField f in fields where f.Field.Type == FieldType.Mail && f.Value != null && !String.IsNullOrEmpty(f.Value.Text) select f.Value.Text).FirstOrDefault();
                                ...
                                {
                                    lm.Comol.Core.Mail.MailService mailService = new lm.Comol.Core.Mail.MailService(smtpConfig, submitterTemplate.MailSettings);
                                    lm.Comol.Core.Mail.dtoMailMessage message = new lm.Comol.Core.Mail.dtoMailMessage(AnalyzeContent(submission.Call, submitterTemplate.MailSettings.IsBodyHtml, submitterTemplate.Subject, submission, submitter, fields, websiteUrl, translations), AnalyzeContent(submission.Call, submitterTemplate.MailSettings.IsBodyHtml, submitterTemplate.Body, submission, submitter, fields, websiteUrl, translations));
                                    message.FromUser = smtpConfig.GetSystemSender();
                                    mailService.SendMail(idUserLanguage, dLanguage, message, mailField, lm.Comol.Core.MailCommons.Domain.RecipientType.To);
                                }
                            }
                        }
                    }
                    ...
                    private String AnalyzeContent(BaseForPaper call, Boolean isHtml, String content, UserSubmission submission, litePerson submitter, List<dtoSubmissionValueField> fields, string webSiteurl, Dictionary<SubmissionTranslations, string> translations)
                    {
                        if (!String.IsNullOrEmpty(content))
                        {
                            ...
                            {
                                content = AnalyzeContent(content, submitter, PlaceHoldersType.SubmitterName, FieldType.Name, fields);
                                content = AnalyzeContent(content, submitter, PlaceHoldersType.SubmitterSurname, FieldType.Surname, fields);
                                content = AnalyzeContent(content, submitter, PlaceHoldersType.SubmitterMail, FieldType.Mail, fields);
                                content = AnalyzeContentForAllFields(isHtml, content, submitter, fields, translations);
                            }
                            if (submission!= null && submission.SubmittedOn.HasValue)
                                dayComplete = submission.SubmittedOn.Value;
                            if (submission != null)
                            {
                                content = content.Replace(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.SubmissionDay), dayComplete.ToString("dd  MMMM  yyyy"));
                                content = content.Replace(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.SubmissionTime), dayComplete.ToShortTimeString());
                                if (content.Contains(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.SubmitterType)) && submission.Type != null)
                                    content = content.Replace(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.SubmitterType), submission.Type.Name);
                            }
                            if (content.Contains(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.CallPublicUrl)) )
                                content = content.Replace(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.CallPublicUrl), webSiteurl + RootObject.StartNewSubmission(call.Type, call.Id, true, false, CallStatusForSubmitters.None, -1));
                            
                            // DA CONTROLLARE
                            if (content.Contains(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.CallInternalUrl)) && submission != null )
                                content = content.Replace(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.CallInternalUrl), webSiteurl + RootObject.StartNewSubmission(call.Type, call.Id,false, false, CallStatusForSubmitters.None, -1));

                            if (content.Contains(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.SubmissionUrl)) && submission != null ){
                                String submissionUrl = "";
                                if (submission.Status >= SubmissionStatus.submitted || submission.Status == SubmissionStatus.deleted || submission.Deleted != BaseStatusDeleted.None)
                                    submissionUrl = RootObject.ViewSubmission(call.Type, call.Id, submission.Id, submission.UserCode, (call.IsPublic && submission.isAnonymous), false, CallStatusForSubmitters.Submitted, (call.IsPortal) ? 0 : -1);
                                else
                                    submissionUrl = RootObject.ContinueSubmission(call.Type, call.Id, (call.IsPublic && submission.isAnonymous), submission.Id, false, (call.Status == CallForPaperStatus.SubmissionClosed || call.Status== CallForPaperStatus.SubmissionsLimitReached  || (call.EndDate.HasValue && call.EndDate.Value < DateTime.Now)) ? CallStatusForSubmitters.SubmissionClosed : CallStatusForSubmitters.SubmissionOpened, (call.IsPortal) ? 0 : -1);
                                content = content.Replace(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.SubmissionUrl), webSiteurl + submissionUrl);
                            }                            
             
                            if (content.Contains(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.CallCommunity)))
                            {
                                liteCommunity community = (submission.Call != null) ? GetCallCommunity(submission.Call.Id) : null;
                                content = content.Replace(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.CallCommunity), (community == null) ? "" : community.Name);
                            }
                            if (content.Contains(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.CallEdition)) || content.Contains(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.CallName)))
                            {
                                content = content.Replace(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.CallEdition), (submission.Call == null) ? "" : submission.Call.Edition);
                                content = content.Replace(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.CallName), (submission.Call == null) ? "" : submission.Call.Name);
                            }
                            if (fields!= null && fields.Select(f => f.IdField).Where(f => content.Contains("[Field_" + f.ToString() + "]") || content.Contains("[Value_" + f.ToString() + "]")).Any())
                    ...
                    private String AnalyzeContentForAllFields(Boolean isHtml, String content, litePerson submitter, List<dtoSubmissionValueField> fields, Dictionary<SubmissionTranslations, string> translations)
                    {
                        if (fields != null && content.Contains(TemplatePlaceHolders.GetPlaceHolder(PlaceHoldersType.AllFields)))
                        {
                            String allFields = "";
                            foreach (dtoSubmissionValueField fieldValue in fields)
                            {
                                String value = (isHtml) ? "<b>{0}</b>: {1}" : "{0}: {1}";
                                switch (fieldValue.Field.Type)
                                {
                                    case FieldType.FileInput:
                                        Boolean uploaded = (fieldValue.Value != null && fieldValue.Value.IdLink>0);
                                        SubmissionTranslations tItem = (uploaded) ? SubmissionTranslations.FileSubmitted : SubmissionTranslations.FileNotSubmitted;
                                        value = String.Format(value,fieldValue.Field.Name, (translations.ContainsKey((tItem))  ? translations[tItem] : "//"));
                                        break;
                                    case FieldType.Disclaimer:
                                        switch (fieldValue.Field.DisclaimerType) { 
                                            case DisclaimerType.None:
                                                value = String.Format(value, fieldValue.Field.Name, "");
                                                break;
                                            case DisclaimerType.CustomDisplayOnly:
                                                value = String.Format(value, fieldValue.Field.Name, translations[SubmissionTranslations.DisclaimerRead]);
                                                break;
                                            case DisclaimerType.Standard:
                                                Boolean response = false;
                                                if (fieldValue.Value != null )
                                                    Boolean.TryParse(fieldValue.Value.Text, out response);
                                                value = String.Format(value, fieldValue.Field.Name, (response) ? translations[SubmissionTranslations.DisclaimerAccept] : translations[SubmissionTranslations.DisclaimerReject]);
                                                break;
                                            case DisclaimerType.CustomSingleOption:
                                            case DisclaimerType.CustomMultiOptions:
                                                String disclaimerValue = "";
                                                if (fieldValue.Value !=null && !String.IsNullOrEmpty(fieldValue.Value.Text))
                                                {
                                                    ...
                                                    foreach (String v in values)
                                                    {
                                                        ...
                                                        FieldOption opt = multipleField.Options.Where(o => o.Deleted == BaseStatusDeleted.None && o.Id.ToString() == v).FirstOrDefault();
                                                        if (opt != null)
                                                            disclaimerValue = disclaimerValue + ((isHtml) ? "<li>" : "") + ((opt.IsFreeValue) ? String.Format(translations[SubmissionTranslations.OtherOptionValue], opt.Name.ToString(), fieldValue.Value.Text) : opt.Name.ToString()) + ((isHtml) ? "</li>" : " ;<br>");
                                                    }
                                                    if (isHtml)
                                                    ...
                                                    disclaimerValue = "//";
                                                value = String.Format(value, fieldValue.Field.Name, (fieldValue.Value == null || String.IsNullOrEmpty(fieldValue.Value.Text)) ? "//" : disclaimerValue);
                                                break;
                                    ...
                                    case FieldType.RadioButtonList:
                                        ...
                                        if (fieldValue.Value !=null && !String.IsNullOrEmpty(fieldValue.Value.Text))
                                        {
                                            ...
                                            str = "//";
                                        value = String.Format(value, fieldValue.Field.Name, (fieldValue.Value==null || String.IsNullOrEmpty(fieldValue.Value.Text)) ? "//" : str);
                                        break;
                                    case FieldType.Note:
                                        value = "";
                                        break;
                                    default:
                                        value = String.Format(value, fieldValue.Field.Name, (fieldValue.Value == null || String.IsNullOrEmpty(fieldValue.Value.Text)) ? "//" : fieldValue.Value.Text);
                                        break;
                                }
                            ...
                            try
                            {
                                ...
                                if (submission.Call.AcceptRefusePolicy == NotifyAcceptRefusePolicy.All || (status == SubmissionStatus.accepted && submission.Call.AcceptRefusePolicy == NotifyAcceptRefusePolicy.Accept) || (status == SubmissionStatus.rejected && submission.Call.AcceptRefusePolicy == NotifyAcceptRefusePolicy.Refuse))
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(mailField))
                                    {
                                        ...
                                        body = body.Replace("[LinkUrl]", webSiteurl + RootObject.ViewSubmission(submission.Call.Type, submission.Call.Id, submission.Id, GetIdLastActiveRevision(submission.Id), submission.UserCode, false, false, CallStatusForSubmitters.Submitted, -1));
                ...
                #region "8 - Revision"
                    ...
                    public Boolean RemoveSelfRequest(CallForPaperType type, long idSubmission, long idRevision, Int32 idUser,Int32 idContainerCommunity, dtoRevisionMessage selfRemoveMessage, string webSiteurl)
                    {
                        ...
                        try
                        {
                            ...
                            if (removed && rev.Submission != null && rev.Submission.Call != null)
                                RevisionMailNotification(rev.Submission, rev, person, rev.Submission.Call.CreatedBy, "", webSiteurl, RootObject.ViewSubmission(type, rev.Submission.Call.Id, idSubmission, false, CallStatusForSubmitters.Submitted, idContainerCommunity), selfRemoveMessage);
                        }
                        catch (Exception ex)
                        {
                            Manager.RollBack();
                    ...
                    public Boolean RemoveUserRequest(CallForPaperType type, long idSubmission, long idRevision, Int32 idUser,Int32 idContainerCommunity, string reason, dtoRevisionMessage managerMessage, String webSiteurl)
                    {
                        ...
                        try
                        {
                            ...
                            if (removed && rev.Submission != null && rev.RequiredBy != null && (rev.RequiredBy.TypeID != (Int32)UserTypeStandard.PublicUser && rev.RequiredBy.TypeID != (Int32)UserTypeStandard.Guest))
                                RevisionMailNotification(rev.Submission, rev, person, rev.RequiredBy, reason, webSiteurl, RootObject.ViewSubmission(type, rev.Submission.Call.Id, idSubmission, false, CallStatusForSubmitters.Submitted, idContainerCommunity), managerMessage);
                        }
                        catch (Exception ex)
                        {
                            Manager.RollBack();
                    ...
                    public Boolean RemoveManagerRequest(CallForPaperType type, long idSubmission, long idRevision, Int32 idUser, Int32 idContainerCommunity, string reason, dtoRevisionMessage managerMessage, String webSiteurl)
                    {
                        ...
                        try
                        {
                            ...
                            if (removed && rev.Submission != null && rev.Submission.Owner != null && (rev.Submission.Owner.TypeID != (Int32)UserTypeStandard.PublicUser && rev.Submission.Owner.TypeID != (Int32)UserTypeStandard.Guest))
                                RevisionMailNotification(rev.Submission, rev, person, rev.Submission.Owner, reason, webSiteurl, RootObject.ViewSubmission(type, rev.Submission.Call.Id, idSubmission, false, CallStatusForSubmitters.Submitted, idContainerCommunity), managerMessage);
                        }
                        catch (Exception ex)
                        {
                            Manager.RollBack();
                    ...
                    public Boolean RefuseUserRequest(CallForPaperType type, Int32 idUser, Int32 idContainerCommunity, long idSubmission, long idRevision, string reason, dtoRevisionMessage message, string webSiteurl)
                    {
                        ...
                        try
                        {
                            ...
                            if (removed && rev.Submission != null && rev.Submission.Owner != null && (rev.Submission.Owner.TypeID != (Int32)UserTypeStandard.PublicUser && rev.Submission.Owner.TypeID != (Int32)UserTypeStandard.Guest))
                                RevisionMailNotification(rev.Submission, rev, person, rev.Submission.Owner, reason, webSiteurl, RootObject.ViewSubmission(type, rev.Submission.Call.Id, idSubmission, false, CallStatusForSubmitters.Submitted, idContainerCommunity), message);
                        }
                        catch (Exception ex)
                        {
                            Manager.RollBack();
        ...
        #region "Submissions management"
            ...
            public List<dtoSubmissionDisplayItemPermission> GetSubmissionList(Boolean allowManage, long idCall, dtoSubmissionFilters filters, int pageIndex, int pageSize)
            {
                ...
                try
                {
                    ...
                    submissions = (from s in items
                                   select new dtoSubmissionDisplayItemPermission() { Id = s.Id, Deleted = s.Deleted, Submission = s, Permission = new dtoSubmissionDisplayPermission(s, allowManage, filters.CallType) }).ToList();
                }
                catch (Exception ex)
                {
            ...
            private List<dtoSubmissionDisplay> GetSubmissionList( long idCall, dtoSubmissionFilters filters, int pageIndex, int pageSize)
            {
