...
namespace lm.Comol.Modules.CallForPapers.Business
{
    public partial class ServiceEvaluation
    {
        ...
        #region "Evaluations"
            ...
            public Evaluation SaveEvaluation(long idEvaluation,long idEvaluator,List<dtoCriterionEvaluated> criteria, String comment, Boolean completed)
            {
                ...
                try
                {
                    ...
                    if (evaluation!=null && person!= null && (person.TypeID!= (int)(UserTypeStandard.PublicUser) && person.TypeID!= (int)(UserTypeStandard.Guest))){
                        ...
                        {
                            List<double> standardValues = (from c in criteria where c.IsValidForCriterionSaving select (double)c.DecimalValue).ToList();
            ...
            #region "Export"
                public dtoEvaluatorCommitteeStatistic GetEvaluatorCommitteeStatistics(EvaluationType type, dtoEvaluationsFilters filters, String anonymousDisplayName, String unknownUserName, long idCommittee, long idEvaluator, Boolean applyFilters = true)
                {
                ...
                public List<dtoEvaluatorCommitteeStatistic> GetEvaluatorCommitteesStatistics(EvaluationType type, dtoEvaluationsFilters filters, String anonymousDisplayName, String unknownUserName, List<long> idCommittees, long idEvaluator, Boolean applyFilters = true)
                {
                    ...
                    if (idCommittees == null && !idCommittees.Any())
                        ...
                        foreach (long idCommittee in idCommittees) {
                            dtoEvaluatorCommitteeStatistic statistic = GetEvaluatorCommitteeStatistics(type, filters, anonymousDisplayName, unknownUserName, idCommittee, idEvaluator, applyFilters);
                            if (statistic != null)
            ...
            #region "Summary"
                ...
                public List<dtoEvaluationSummaryItem> GetEvaluationsList(long idCall,EvaluationType type, dtoEvaluationsFilters filters, String anonymousUser, String unknownUser, Boolean loadRevisionInfo)
                {
                    List<dtoEvaluationSummaryItem> items = null;
                    try{
                        List<DssCallEvaluation> dssRatings = DssRatingGetValues(idCall);
                        List<expEvaluation> evaluations = (from e in Manager.GetIQ<expEvaluation>() where e.Deleted == BaseStatusDeleted.None && e.IdCall == idCall select e).ToList();
                        List<long> idSubmissions = evaluations.Select(e => e.IdSubmission).Distinct().ToList();
                        List<dtoBaseSummaryItem> temp = GetBaseEvaluationDisplayItems(idSubmissions, anonymousUser, unknownUser, loadRevisionInfo);

                        items = temp.Where(i => idSubmissions.Contains(i.IdSubmission)).Select(i => new dtoEvaluationSummaryItem(i, evaluations.Where(e => e.IdSubmission == i.IdSubmission && e.Deleted == BaseStatusDeleted.None).ToList(), dssRatings.Where(e=> e.IdSubmission==i.IdSubmission).FirstOrDefault())).ToList();
                ...
                private List<dtoBaseSummaryItem> GetBaseEvaluationDisplayItems(List<long> idSubmissions, String anonymousUser, String unknownUser, Boolean loadIdRevision)
                {
                ...
                public String ExportSummaryStatistics(dtoCall call, dtoEvaluationsFilters filters, String anonymousDisplayName, String unknownUserDisplayName, SummaryType summaryType, ItemsToExport itemsToExport, ExportData exportData, lm.Comol.Core.DomainModel.Helpers.Export.ExportFileType fileType, Dictionary<lm.Comol.Modules.CallForPapers.Domain.Evaluation.EvaluationTranslations, String> translations, Dictionary<lm.Comol.Modules.CallForPapers.Domain.Evaluation.EvaluationStatus, String> status, long idCommittee = 0)
                {
                ...
                public List<dtoSubmissionCommitteeItem> GetSubmissionEvaluations(long idCall,long idSubmission, long idCommittee, String unknownUser)
                {
                    ...
                    try
                    {
                        ...
                        foreach (var c in evaluations.GroupBy(e => e.Committee)) {
                            ...
                            item.Criteria = (c.Key.Criteria != null) ? c.Key.Criteria.OrderBy(cr => cr.DisplayOrder).ThenBy(cr => cr.Name).Select(cr => new dtoCriterionSummaryItem(cr, members.Count)).ToList() : new List<dtoCriterionSummaryItem>();
