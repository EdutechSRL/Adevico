    ...
    public class RootObject
    {
        ...
        #region "NEW"
            ...
            #region "View Submission"
                ...
                public static string ViewSubmission(CallForPaperType type, long idCall, long idSubmission, Boolean fromPublicList, CallStatusForSubmitters view, Int32 containerIdCommunity)
                {
                    return ViewSubmission(type, idCall, idSubmission, 0, Guid.Empty, false, fromPublicList, view, containerIdCommunity);
                }
                public static string ViewSubmission(CallForPaperType type, long idCall, long idSubmission, System.Guid uniqueId, Boolean isPublic, Boolean fromPublicList, CallStatusForSubmitters view, Int32 containerIdCommunity)
                {
                    return ViewSubmission(type, idCall, idSubmission, 0, uniqueId, isPublic, fromPublicList, view, containerIdCommunity);
                }
                public static string ViewSubmission(CallForPaperType type, long idCall, long idSubmission, long idRevision, System.Guid uniqueId, Boolean isPublic, Boolean fromPublicList, CallStatusForSubmitters view, Int32 containerIdCommunity)
                {
                    String baseurl = "Modules/CallForPapers/" + ((isPublic) ? "Public" : "") + "Submission.aspx?";
                    return baseurl + "type=" + type.ToString() + "&idCall=" + idCall.ToString() + "&idSubmission=" + idSubmission.ToString() + ((idRevision == 0) ? "" : "&idRevision=" + idRevision.ToString()) + ((uniqueId == System.Guid.Empty) ? "" : "&uniqueId=" + uniqueId.ToString()) + "&View=" + view.ToString() + ((fromPublicList == true) ? "&FromPublicList=true" : "") + ((containerIdCommunity > -1) ? "&idOtherCommunity=" + containerIdCommunity.ToString() : "");
                }
                public static string ViewSubmissionAsManager(CallForPaperType type, long idCall, long idSubmission, long idRevision, System.Guid uniqueId, Boolean isPublic, CallStatusForSubmitters view, SubmissionFilterStatus filter, SubmissionsOrder order, Boolean ascending, int pageIndex, int pageSize)
                {
                    return ViewSubmission(type, idCall, idSubmission, idRevision, uniqueId, isPublic, false, view, -1) + "&Manage=true&filter=" + filter.ToString() + "&order=" + order.ToString() + "&ascending=" + ascending.ToString() + "&pageIndex=" + pageIndex.ToString() + "&pageSize=" + pageSize.ToString();
                }
            #endregion
                #region "Editing"
        ...
        #region "Evaluations"
            ...
            #region "Evaluating"
                ...
                private static string ViewEvaluation(long idCall, int idCommunity)
                {
                    return "Modules/CallForPapers/Statistics/ViewEvaluation.aspx?idCommunity=" + idCommunity.ToString() + "&idCall=" + idCall.ToString();
                }
                public static string ViewSubmissionEvaluations(long idSubmission, long idCall, int idCommunity)
                {
                    return ViewEvaluation(idCall,idCommunity) + "&idSubmission=" + idSubmission.ToString();
                }
                public static string ViewUserEvaluations(long idEvaluator, long idSubmission, long idCall, int idCommunity)
                {
                    return ViewSubmissionEvaluations(idSubmission, idCall,idCommunity) + "&idEvaluator=" + idEvaluator.ToString();
                }
                public static string ViewSingleEvaluation(long idEvaluation, long idSubmission, long idCall, int idCommunity)
                {
                    return ViewSubmissionEvaluations(idSubmission, idCall, idCommunity) + ((idEvaluation <= 0) ? "" : "&idEvaluation=" + idEvaluation.ToString());
                }
