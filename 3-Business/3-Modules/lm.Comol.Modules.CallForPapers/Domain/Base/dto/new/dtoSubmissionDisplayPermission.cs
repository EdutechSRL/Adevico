...
namespace lm.Comol.Modules.CallForPapers.Domain
{
    ...
    public class dtoSubmissionDisplayPermission
    {
        ...
        public dtoSubmissionDisplayPermission(dtoSubmissionDisplay submission, Boolean manage, CallForPaperType type){
            Delete = (submission.Deleted != BaseStatusDeleted.None && (type!= CallForPaperType.CallForBids || submission.Status== SubmissionStatus.draft)); // && type != CallForPaperType.CallForBids);
            VirtualDelete = (submission.Deleted == BaseStatusDeleted.None && (type != CallForPaperType.CallForBids || submission.Status == SubmissionStatus.draft || submission.Status == SubmissionStatus.submitted)) && manage;
