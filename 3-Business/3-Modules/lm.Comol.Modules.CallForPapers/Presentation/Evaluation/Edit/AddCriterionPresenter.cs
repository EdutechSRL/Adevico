...
namespace lm.Comol.Modules.CallForPapers.Presentation.Evaluation
{
    public class AddCriterionPresenter : lm.Comol.Core.DomainModel.Common.DomainPresenter
    {
        ...
        public void InitView(long idCall, Boolean useDss, lm.Comol.Core.Dss.Domain.Templates.dtoItemMethodSettings settings = null, lm.Comol.Core.Dss.Domain.Templates.dtoSelectMethod method =null)
        {
            View.UseDss = useDss;
            View.IdCall = idCall;
            ...
            else
            {
                List<CriterionType> types = (from e in Enum.GetValues(typeof(CriterionType)).Cast<CriterionType>().ToList()
                                             where e != CriterionType.None && e != CriterionType.Textual &&
