            ...
            if (UserContext.isAnonymous)
            {
                switch (dEvaluation) {
                    case DisplayEvaluations.Single:
                        View.DisplaySessionTimeout(RootObject.ViewSingleEvaluation(idEvaluation,idSubmission,idCall,idCommunity));
                        break;
                    case DisplayEvaluations.ForUser:
                        View.DisplaySessionTimeout(RootObject.ViewUserEvaluations(idEvaluator, idSubmission, idCall, idCommunity));
                        break;
                    default:
                        View.DisplaySessionTimeout(RootObject.ViewSubmissionEvaluations( idSubmission, idCall, idCommunity));
                        break;
                }
            }
            else
        ...
        private void LoadData(DisplayEvaluations display, Int32 idCommunity, dtoCall call, dtoSubmissionRevision submission, long idEvaluation, long idEvaluator)
        {
            ...
            switch (display) { 
                case DisplayEvaluations.ForSubmission:
                    ...
                    View.LoadEvaluations(Service.GetSubmissionEvaluations(submission.Id, call.Id,View.UnknonwUserName));
                    break;
                case DisplayEvaluations.ForUser:
                    committees = Service.GetCommitteesInfoForEvaluator(idEvaluator);
                    ...
                    break;
                case DisplayEvaluations.Single:
                    lm.Comol.Modules.CallForPapers.Domain.Evaluation.dtoEvaluation evaluation = Service.GetFullEvaluation(idEvaluation, View.AnonymousDisplayName, View.UnknonwUserName);
                    if (evaluation == null)
                        View.DisplayUnknownEvaluation(View.IdCallCommunity, View.IdCallModule, idEvaluation);
                    else {
                        dtoCommitteeEvaluation result = new dtoCommitteeEvaluation(){ Evaluation=evaluation, IdCommittee = evaluation.IdCommittee};
                        committees = Service.GetCommitteesInfoForEvaluator(evaluation.IdEvaluator);
