...
namespace lm.Comol.Modules.CallForPapers.Presentation.Evaluation
{
    public class SubmissionsToEvaluatePresenter : lm.Comol.Core.DomainModel.Common.DomainPresenter
    {
        ...
        private void InitalizeView(long idCall,Int32 idCommunity, long idCommittee, long idEvaluator,dtoEvaluationsFilters filters )
        {
            LoadCommitteData(idCall,idCommunity, idCommittee, idEvaluator, InitializeFilters(idCall,idCommittee,idEvaluator, filters));
        }

        private dtoEvaluationsFilters InitializeFilters(long idCall, long idCommittee, long idEvaluator, dtoEvaluationsFilters filters)
        {
            List<EvaluationFilterStatus> items = Service.GetAvailableEvaluationFilterStatus(idCall, idCommittee, idEvaluator);
        ...
        {
            List<dtoCommitteeEvaluationsInfo> committees = Service.GetCommitteesEvaluationInfo(idCall, idEvaluator);
        ...
        {
            return Service.ExportEvaluatorStatistics(ServiceCall.GetDtoCall(View.IdCall), filters, View.AnonymousDisplayname, View.UnknownDisplayname, idCommittee, idEvaluator, applyFilters, fileType, translations, status);
        }
