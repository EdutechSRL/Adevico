                    ...
                    if (submission.Owner.Id == idUser || forPublicCall) {
                        if (idRevision >= 1) {
                            ...
                            if (rev==null || (rev!= null && (rev.Type== RevisionType.Original || rev.Status==  RevisionStatus.Approved)))
                                View.SetActionUrl(CallStandardAction.ViewOwnSubmission, RootObject.ViewSubmission(type, idCall, idSubmission, idRevision, uniqueID, call.IsPublic && submission.IsAnonymous, View.FromPublicList, CallStatusForSubmitters.Submitted, (hasPermissionToSubmit && idCommunity > 0 && containerIdCommunity > 0 && idCommunity != containerIdCommunity) ? containerIdCommunity : idCommunity));
                            else
