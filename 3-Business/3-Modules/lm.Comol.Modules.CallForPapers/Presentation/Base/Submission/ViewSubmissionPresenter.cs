...
namespace lm.Comol.Modules.CallForPapers.Presentation
{
    public class ViewSubmissionPresenter : lm.Comol.Core.DomainModel.Common.DomainPresenter
    {
            ...
            if (submission==null || submission.Owner==null )
                ...
                else if (isAnonymousUser && !forPublicCall)
                {
                    if (call.IsPublic && submission.IsAnonymous )
                        View.GoToUrl(RootObject.ViewSubmission(type, idCall, idSubmission, idRevision, uniqueId, true, false, View.PreloadView, View.PreloadIdOtherCommunity));
                    else
        ...
        private void LoadSections(BaseForPaper call, dtoSubmissionRevision subRev, Boolean allowAdmin)
        {
            ...
            View.InitializeExportControl((subRev.Owner !=null && subRev.Owner.Id == UserContext.CurrentUserID), (subRev.Owner !=null) ? subRev.Owner.Id : 0,call.Id, subRev.Id, View.IdRevision, View.IdCallModule, View.IdCallCommunity, View.CallType);
        }
            ...
            try
            {
                if (UserContext.isAnonymous)
                {
                    if (View.isAnonymousSubmission)
                        View.GoToUrl(RootObject.ViewSubmission(View.CallType, View.IdCall, View.IdSubmission, View.IdRevision, View.PreloadedUniqueID, true, false, View.PreloadView, View.PreloadIdOtherCommunity));
                    else
                        View.DisplaySessionTimeout();
                }
                else
        ...
        private void LoadRevisionInfos(Boolean allowAdmin, BaseForPaper call, dtoSubmissionRevision submission)
        {
            ...
            Boolean submissionRevision = (allowRevision && submission.Status >= SubmissionStatus.submitted && submission.Status != SubmissionStatus.rejected) ;
        ...
        public void AskForRevision(long idSubmission, string reason, dtoRevisionMessage message, String webSiteurl)
        {
            ...
            if (!UserContext.isAnonymous)
            {
                Revision rev = ServiceCall.AskForRevision(idSubmission, idUser, reason,webSiteurl, RootObject.ViewSubmission(View.CallType,View.IdCall, View.IdSubmission,false, CallStatusForSubmitters.Submitted,View.PreloadIdOtherCommunity), message);
                if (rev != null)
