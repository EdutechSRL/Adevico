...
namespace lm.Comol.Core.BaseModules.FileRepository.Business
{
    public partial class ServiceRepositoryScorm : ServiceRepository, iLinkedNHibernateService
    {
        ...
        #region Ignore
            ...
            public void SaveActionExecution(ModuleLink link, bool isStarted, bool isPassed, short Completion, bool isCompleted, short mark, int idUser, Dictionary<string, long> moduleUserLong = null, Dictionary<string, string> moduleUserString = null)
            {
        ...
        public List<dtoItemEvaluation<long>> EvaluateModuleLinks(List<ModuleLink> links, int idUser, Dictionary<string, long> moduleUserLong = null, Dictionary<string, string> moduleUserString = null)
        {
            ...
            if (links.Any()){
                List<dtoItemToEvaluate> items = links.Where(l => l.Action == (Int32)ModuleRepository.ActionType.DownloadFile).Select(l => new dtoItemToEvaluate(l.Id, l.DestinationItem.ObjectLongID, l.DestinationItem.ObjectIdVersion)).ToList();
                List<long> idItems = items.Where(i => i.AlwaysLastVersion).Select(i => i.IdItem).Distinct().ToList();
                List<long> idVersions = items.Where(i => !i.AlwaysLastVersion).Select(i => i.IdVersion).Distinct().ToList();
                if (idItems.Any())
                {
                    idItems = DownloadedItems(idItems, idUser);
                    results.AddRange((from i in items where idItems.Contains(i.IdItem) select new dtoItemEvaluation<long>() { Completion = 100, isCompleted = true, isPassed = true, isStarted = true, Item = i.Idlink, IdLink = i.Idlink }).ToList());
                }

                if (idVersions.Any())
                {
                    idVersions = DownloadedVersions(idVersions, idUser);
                    results.AddRange((from i in items where idVersions.Contains(i.IdVersion) select new dtoItemEvaluation<long>() { Completion = 100, isCompleted = true, isPassed = true, isStarted = true, Item = i.Idlink, IdLink = i.Idlink }).ToList());
                }


                items = links.Where(l => l.Action == (Int32)ModuleRepository.ActionType.PlayFile).Select(l => new dtoItemToEvaluate(l.Id, l.DestinationItem.ObjectLongID, l.DestinationItem.ObjectIdVersion)).ToList();
                List<long> idItemsPlay = items.Where(i => i.AlwaysLastVersion).Select(i => i.IdItem).Distinct().ToList();
                List<long> idVersionPlay = items.Where(i => !i.AlwaysLastVersion).Select(i => i.IdVersion).Distinct().ToList();
                if (idItemsPlay.Any())
                {
                    List<liteRepositoryItem> rItems = GetQuery<liteRepositoryItem>().Where(i => idItemsPlay.Contains(i.Id)).ToList();
                    if (rItems.Any(i => i.Type == ItemType.Multimedia))
                    {
                        idItems = rItems.Select(i => i.Id).Distinct().ToList();
                        idItems = (from p in GetQuery<PlayStatistics>() where idItems.Contains(p.IdItem) && p.IdPerson == idUser select p.IdItem).Distinct().ToList();
                        results.AddRange((from i in items where idItems.Contains(i.IdItem) select new dtoItemEvaluation<long>() { Completion = 100, isCompleted = true, isPassed = true, isStarted = true, Item = i.Idlink, IdLink = i.Idlink }).ToList());
                    }
                    if (rItems.Any(i => i.Type == ItemType.ScormPackage))
                    {
                        ...
                        foreach(liteRepositoryItem item in rItems){
                            dtoPackageEvaluation eval = ScormService.GetPackageEvaluation_NewTMP(idUser, item.Id, item.UniqueId, item.UniqueIdVersion, Modules.ScormStat.Domain.UserStatisticsFrom.Last, Modules.ScormStat.Domain.PlayCompletionStatus.Ignore, date);
                            ...
                            results.AddRange((from i in items where i.IdItem == item.Id select new dtoItemEvaluation<long>() { Mark = (short)mark, Completion = (short)completion, isCompleted = isCompleted, isPassed = isPassed, isStarted = isStarted, Item = i.Idlink, IdLink = i.Idlink }).ToList());
                        }
                ...
                {
                    List<liteRepositoryItemVersion> vItems = GetQuery<liteRepositoryItemVersion>().Where(i => idVersionPlay.Contains(i.Id)).ToList();
                    if (vItems.Any(i => i.Type == ItemType.Multimedia))
                    {
                        idVersions = vItems.Select(i => i.Id).Distinct().ToList();
                        idVersions = (from p in GetQuery<PlayStatistics>() where idVersions.Contains(p.IdVersion) && p.IdPerson == idUser select p.IdVersion).Distinct().ToList();
                        results.AddRange((from i in items where idVersions.Contains(i.IdVersion) select new dtoItemEvaluation<long>() { Completion = 100, isCompleted = true, isPassed = true, isStarted = true, Item = i.Idlink, IdLink = i.Idlink }).ToList());
                    }
