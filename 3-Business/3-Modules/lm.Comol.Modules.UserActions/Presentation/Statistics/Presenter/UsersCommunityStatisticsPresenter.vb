...
Namespace lm.Comol.Modules.UserActions.Presentation
    Public Class UsersCommunityStatisticsPresenter
...
#Region "Standard"
        ...
        Public Overloads Property CurrentManager() As ManagerUsageStatistic
            ...
            End Set
        End Property
        Public Overloads ReadOnly Property View() As IViewUsersCommunityStatistics
            Get
                Return MyBase.View
            End Get
        End Property
        ...
        End Sub
        Public Sub New(ByVal oContext As iApplicationContext, ByVal view As IViewUsersCommunityStatistics)
...
#End Region
        ...
        Public Sub InitView()
            If Not Me.UserContext.isAnonymous Then
                ...
                Else
                    ...
                    ElseIf items.Contains(StatisticView.UsersCommunity) Then
                        LoadUsers(oContext, p)
                    ...
                    Else
                        Select Case items.FirstOrDefault()
                            Case StatisticView.Community
                                View.LoadView(RootObject.CommunityStatistics(0, oContext.CommunityID, StatisticOrder.ModuleName, True, View.PreloadedFromView))
                            Case Else
            ...
            End If
        End Sub
        Public Sub LoadUsers(ByVal oUsage As UsageContext, p As ModuleStatistics)
            ...
            oSummary.NavigateTo = RootObject.UsageDetails(oSummaryContext.UserID, oSummaryContext.CommunityID, DetailViewType.Month, StatisticView.UsersCommunity, View.CurrentUrl)
            ...
            For Each o In oStatistic.Items
                ...
                o.NavigateTo = RootObject.UserCommunity(0, o.ID, oUsage.CommunityID, StatisticOrder.ModuleName, True, backUrl, StatisticView.UsersCommunity)
            Next
            Me.View.LoadItems(oStatistic, oUsage, Me.View.CurrentView, IViewUsageDetails.viewType.Day)

            View.SendAction(UserContext.CurrentCommunityID, UserContext.CurrentUserID, oUsage.CommunityID, ModuleStatistics.ObjectType.Community, ModuleStatistics.ActionType.LoadCommunityUsersStatistics)
        End Sub
        ...
        Private Function GetDefaultContext() As UsageContext
            ...
            If oContext.Order = StatisticOrder.None Then
                oContext.Order = StatisticOrder.Owner
