...
Namespace lm.Comol.Modules.UserActions.Presentation
    Public Class UserSystemStatisticsPresenter
...
#Region "Standard"
        ...
        Public Overloads Property CurrentManager() As ManagerUsageStatistic
            ...
            End Set
        End Property
        Public Overloads ReadOnly Property View() As IViewUserSystemStatistics
            Get
                Return MyBase.View
            End Get
        End Property
        ...
        End Sub
        Public Sub New(ByVal oContext As iApplicationContext, ByVal view As IViewUserSystemStatistics)
...
#End Region
        ...
        Public Sub InitView()
            If Not Me.UserContext.isAnonymous Then
                ...
                If idType = UserTypeStandard.Administrative OrElse idType = UserTypeStandard.Administrator OrElse idType = UserTypeStandard.SysAdmin Then
                    ...
                    Dim p As Person = BaseDomainManager.GetPerson(oContext.UserID)
                    If IsNothing(p) Then
                        View.DisplayUnknownUser()
                        View.SendAction(0, UserContext.CurrentUserID, oContext.UserID, ModuleStatistics.ObjectType.User, ModuleStatistics.ActionType.UserNotFound)
                    Else
                        LoadGlobal(oContext)
                    End If
                Else
                ...
                End If
                Dim url As String = View.PreloadedBackUrl
        ...
        Public Sub LoadGlobal(ByVal oUsage As UsageContext)
            ...
            For Each o In oStatistic.Items
                o.NavigateTo = RootObject.UserSystemCommunityStatistics(oUsage.UserID, o.ID, StatisticOrder.ModuleName, True, View.GetEncodedBackUrl, StatisticView.UserSystem)
            Next
            Me.View.LoadItems(oStatistic, oUsage, Me.View.CurrentView, IViewUsageDetails.viewType.Day)
            View.SendAction(0, UserContext.CurrentUserID, oUsage.UserID, ModuleStatistics.ObjectType.User, ModuleStatistics.ActionType.LoadPortalUserStatistics)
        End Sub
