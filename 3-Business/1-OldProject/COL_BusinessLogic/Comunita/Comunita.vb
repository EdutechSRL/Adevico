...
#Region "Metodi Standard"
		...
		Protected Function AggiungiComunita(ByVal IdCreatore As Integer, ByVal SmsRicezione As Integer, ByVal Iscrivi As Boolean) As Integer
			'se riesce ad aggiungere setta l'id dell'oggetto comunit‡ a quello appena inserito nel db
			...
			Try
				objAccesso.GetExecuteNotQuery(oRequest)
				'assegna all'oggetto l'id della comunit‡ appena creata
				...
				'associa i permessi 
				'TUTTO IN AUTOMATICO:MARMOTTE Ë UN CASINO ALTRIMENTI !!!!
				' Me.AssociaPermessiByRuoloAndOrganizzazione()

			Catch
				' se nn Ë avvenuto l'inserimento setta a -1 l'id della comunit‡
				Me.n_CMNT_id = -1
				n_Errore = Errori_Db.DBInsert
			End Try
		End Function 'inserisce nuova comunit‡ e gli associa chi la crea come responsabile
		Protected Sub ClonaComunita(ByVal CloneID As Integer, ByVal IdCreatore As Integer, ByVal SmsRicezione As Integer, ByVal Iscrivi As Boolean)
			...
			Catch ex As Exception
				...
				n_Errore = Errori_Db.DBInsert
			End Try
		End Sub		 'inserisce nuova comunit‡ e gli associa chi la crea come responsabile
		...
		Public Sub AssociaPadre(ByVal IdPadre As Integer) 'aggiunge un nuovo padre alla comunit‡ 
		...
		Public Sub AssociaPadri(ByVal Condition As String) 'aggiunge alla comunit‡ i padri passati nella lista
...
#End Region
		...
		Public Function GetOrganizzazioneID() As Integer
			'ritorna l'id dell'organizzazione a cui appartiene la comunit‡
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
...
#Region "Metodi File"
		Public Shared Function HasNewFile(ByVal CMNT_Id As Integer, ByVal PRSN_ID As Integer, ByVal isLoggato As Boolean, ByVal oFiltroVisibilita As Main.FiltroVisibilit‡) As Boolean
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
			...
			With oRequest
				...
				oParam = objAccesso.GetParameter("@FiltroVisibilita", CType(oFiltroVisibilita, Main.FiltroVisibilit‡), ParameterDirection.Input, DbType.Int32)
				.Parameters.Add(oParam)
...
#Region "Metodi associazione Servizi"
		...
		Public Function ElencaByPermesso(ByVal IdServizio As Integer, ByVal IdPersona As Integer, ByVal Posizione As Integer, Optional ByVal Abilitazione As Integer = 1, Optional ByVal Attivato As Integer = 1, Optional ByVal Abilitato As Integer = 1, Optional ByVal Valore As Integer = 1, Optional ByVal SRVZ_Attivato As Integer = 1) As DataSet
			'mi ritorna l'elenco delle comunit‡ nelle quali ho un determinato permesso passato in ingresso
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
		...
		Public Function ElencaPadriDirettiByPermesso(ByVal IdServizio As Integer, ByVal IdPersona As Integer, ByVal Posizione As Integer, ByVal IdComunita As Integer, Optional ByVal Abilitazione As Integer = 1, Optional ByVal Attivato As Integer = 1, Optional ByVal Abilitato As Integer = 1, Optional ByVal Valore As Integer = 1, Optional ByVal SRVZ_Attivato As Integer = 1) As DataSet
			'mi ritorna l'elenco delle comunit‡ padri nelle quali ho un determinato permesso che non sono gi‡ associate come padre alla comunit‡ attuale
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
			Dim dsTable As DataSet = Nothing
			Dim objAccesso As New COL_DataAccess
...
#Region "Metodi Gestione Iscritti"
		...
		Public Function SetMaxIscritti(ByVal MaxIscritti As Integer) As Boolean
			...
			Return iResponse
		End Function
		'Funzione che indica,dato un tipo ruolo, quanti iscritti vi sono per la comunit‡

		'aggiunge l'utente in fase di registrazione alla super-comunit‡ di livello 0 corrispondente
		...
		'passo l'id utente che verra iscritti ad una comunit‡ come passanti
...
#Region "Metodi Gestione Ruoli"
		Public Function RuoloByComunitaAndPersona(ByVal IdPersona As Integer) As Integer 'agg al: 30/10/2003
			'ritorna il ruolo della persona in quella comunit‡
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
		...
		Public Function RuoloDefault() As Integer
			'restituisce il tipo ruolo di default con cui c si iscrive ad alla comunit‡
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
			...
			Try
				objAccesso.GetExecuteNotQuery(oRequest)
				' Recupero l'ID del tipo di ruolo di default per quel tipo di comunit‡
		...
		Public Function RuoliAssociati(ByVal LinguaID As Integer, ByVal oFiltro As FiltroRuoli, Optional ByVal oFiltroUtenti As FiltroUtenti = Main.FiltroUtenti.NoPassantiNoCreatori, Optional ByVal EsclusoUtenteID As Integer = 0) As DataSet
			'elenca in un dataset i ruoli che sono assegnati ad una comunit‡
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
...
#Region "Metodi"
        ...
        Public Function ElencaNonIscritti_MaIscrittiACmntPassata(ByVal LinguaID As Integer, ByVal TipoRuolo As Integer, ByVal CMNT_id_passato As Integer, Optional ByVal valore As String = "", Optional ByVal oAnagrafica As FiltroAnagrafica = FiltroAnagrafica.tutti, Optional ByVal oFiltroUtente As Main.FiltroUtenti = Main.FiltroUtenti.NoPassantiNoCreatori, Optional ByVal oFiltroRicerca As Main.FiltroRicercaAnagrafica = Main.FiltroRicercaAnagrafica.tutti, Optional ByVal pFilter As ProfileStatusFilter = ProfileStatusFilter.all) As DataSet
            'elenca gli iscritti ad una comunit‡ passata ma non alla corrente
...
#Region "Gestione Comunit‡ / Cancellazione"
		...
		Public Function GetPadri(ByVal LinguaID As Integer) As DataSet
			'elenca tutte le comunit‡ padri associate ad una data comunit‡
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
			Dim objAccesso As New COL_DataAccess
			Dim oDataSet As New DataSet
...
#Region "Info Comunit‡"
		...
		'        'praticamente serve per avere di ritorno anche la data e ora di creazione della comunit‡
		...
		Public Sub MailNotifica(ByVal oPersona As COL_Persona, ByVal link As String, ByVal SessionId As String, ByVal TPRL_Nome As String, ByVal LinguaCode As String, ByVal oLocalizedMail As MailLocalized)
			...
			Try
				...
				If oMail.IndirizziTO.Count > 0 Then
					...
					If oPersona.TipoPersona.id = 4 Then	'se Ë esterno scrivo la mansione
...
#Region "NUOVE funzioni ricerca"
		...
		'    Return oDs
		'End Function
		'Public Shared Function RicercaCorsoForIscrizione(ByVal LinguaID As Integer, ByVal Facolt‡ID As Integer, Optional ByVal CMNT_IdPadre As Integer = 0, Optional ByVal PRSN_Id As Integer = -1, Optional ByVal oFiltroTipoRicerca As Main.FiltroComunita = Main.FiltroComunita.tutti, Optional ByVal oFiltroLetteraRicerca As Main.FiltroComunita = Main.FiltroComunita.tutti, Optional ByVal valore As String = "", Optional ByVal CorsoDiStudi_Id As Integer = -1, Optional ByVal Anno As Integer = -1, Optional ByVal PeriodoID As Integer = -1, Optional ByVal FiltroStato As Main.FiltroStatoComunita = Main.FiltroStatoComunita.Tutte) As DataSet
		'    Dim oRequest As New COL_Request
		'    Dim oParam As New COL_Request.Parameter
		'    Dim oDataSet As New DataSet
		'    Dim objAccesso As New COL_DataAccess
		...
		'        oParam = objAccesso.GetAdvancedParameter("@ORGN_ID", Facolt‡ID, ParameterDirection.Input, SqlDbType.Int)
		'        .Parameters.Add(oParam)
		...
		Public Shared Function RicercaComunitaForManagement(ByVal LinguaID As Integer, ByVal Facolt‡ID As Integer, Optional ByVal Percorso As String = "", Optional ByVal CMNT_IdPadre As Integer = 0, Optional ByVal PRSN_Id As Integer = -1, Optional ByVal oFiltroTipoRicerca As Main.FiltroComunita = Main.FiltroComunita.tutti, Optional ByVal oFiltroLetteraRicerca As Main.FiltroComunita = Main.FiltroComunita.tutti, Optional ByVal valore As String = "", Optional ByVal TipoComunitaID As Integer = -1, Optional ByVal TipoCorsoDiStudiID As Integer = -1, Optional ByVal CorsoDiStudi_Id As Integer = -1, Optional ByVal Anno As Integer = -1, Optional ByVal PeriodoID As Integer = -1, Optional ByVal FiltroStato As Main.FiltroStatoComunita = Main.FiltroStatoComunita.Tutte, Optional ByVal FiltroIscrizione As FiltroRicercaComunitaByIscrizione = Main.FiltroRicercaComunitaByIscrizione.forAdmin) As DataSet
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
			Dim oDataSet As New DataSet
			Dim objAccesso As New COL_DataAccess
		...
		Public Shared Function RicercaComunitaAlberoForManagement(ByVal LinguaID As Integer, ByVal Facolt‡ID As Integer, Optional ByVal Percorso As String = "", Optional ByVal CMNT_IdPadre As Integer = 0, Optional ByVal PRSN_Id As Integer = -1, Optional ByVal oFiltroTipoRicerca As Main.FiltroComunita = Main.FiltroComunita.tutti, Optional ByVal oFiltroLetteraRicerca As Main.FiltroComunita = Main.FiltroComunita.tutti, Optional ByVal valore As String = "", Optional ByVal TipoComunitaID As Integer = -1, Optional ByVal TipoCorsoDiStudiID As Integer = -1, Optional ByVal CorsoDiStudi_Id As Integer = -1, Optional ByVal Anno As Integer = -1, Optional ByVal PeriodoID As Integer = -1, Optional ByVal FiltroStato As Main.FiltroStatoComunita = Main.FiltroStatoComunita.Tutte, Optional ByVal FiltroIscrizione As FiltroRicercaComunitaByIscrizione = Main.FiltroRicercaComunitaByIscrizione.forAdmin, Optional ByVal ForAlberoAccesso As Boolean = False, Optional ByVal EscludiPath As Boolean = True) As DataSet
			...
			Return oDataSet
		End Function
		Public Shared Function GeneraAlberoComunitaForAccesso(ByVal LinguaID As Integer, ByVal Facolt‡ID As Integer, Optional ByVal Percorso As String = "", Optional ByVal CMNT_IdPadre As Integer = 0, Optional ByVal PRSN_Id As Integer = -1, Optional ByVal oFiltroTipoRicerca As Main.FiltroComunita = Main.FiltroComunita.tutti, Optional ByVal oFiltroLetteraRicerca As Main.FiltroComunita = Main.FiltroComunita.tutti, Optional ByVal valore As String = "", Optional ByVal TipoComunitaID As Integer = -1, Optional ByVal TipoCorsoDiStudiID As Integer = -1, Optional ByVal CorsoDiStudi_Id As Integer = -1, Optional ByVal Anno As Integer = -1, Optional ByVal PeriodoID As Integer = -1, Optional ByVal FiltroStato As Main.FiltroStatoComunita = Main.FiltroStatoComunita.Tutte) As DataSet
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
			Dim oDataSet As New DataSet
			Dim objAccesso As New COL_DataAccess
...
#End Region
		...
		Public Function RuoliAssociabiliByPersonaCol(ByVal PRSN_Id As Integer, ByVal oFiltro As FiltroRuoli, Optional ByVal oFiltroUtenti As FiltroUtenti = Main.FiltroUtenti.NoPassantiNoCreatori) As GenericCollection(Of Role)
			'elenca in una generic collection i ruoli che sono assegnati ad una comunit‡
			Dim oRequest As New COL_Request
			Dim oParam As New COL_Request.Parameter
...
#Region "Lazy Iscritti comunita"
        ...
        ''' </summary>
        ''' <param name="IdComunita">STRINGA: id delle comunit‡ separati va virgola. Devono cominciare a finire per la stessa, ad esempio ,1, o ,1,2,146,</param>
        ''' <param name="IdLingua">ID della lingua per l'internazionalizzazione... (non serve, solo per compatibilit‡ e sviluppi futuri)</param>
        ...
        ''' <remarks>La ricerca viene effettuata da dB, poichË il collo di bottiglia sta nel passaggio di dati tra dB e Server Web</remarks>
        Public Shared Function GetAllIscrittiComunitaMultipleFiltrate( _
                ByVal IdComunita As String, _
                ByVal IdLingua As Integer, _
                ...
                Optional ByVal IdTipo As Integer = -1 _
        ...
        'Applico i filtri all'elenco iscritti di una comunita. Se l'elenco non Ë in cache lo recupero dal dB.
        'Non ancora paginati!!! (III funzione?!=
        Public Shared Function GetIscrittiLazy( _
         ByRef Totale As Integer, _
         ByVal ComunitaID As Integer, _
         ByVal LinguaID As Integer, _
         ByVal IdPersona As Integer, _
         ByVal oFiltroAbilitazione As FiltroAbilitazione, _
         ByVal oFiltroUtente As FiltroUtenti, _
         Optional ByVal TPRL_id As Integer = -1, _
         Optional ByVal Valore As String = "", _
         Optional ByVal oAnagrafica As FiltroAnagrafica = FiltroAnagrafica.tutti, _
         Optional ByVal WithFoto As Boolean = False, _
...
#End Region
        ...
        '                        'oDataReader("CMNT_PRFS_ID") 'e ke Ë sta robba?!?! E' sempre zero... :s
            ...
            Try
                ...
                Try
                    While iDataReader.Read
                        iLista.Add(New ServicePage(LinguaID, iDataReader.Item("DFLP_ID"), GenericValidator.ValString(iDataReader.Item("DFLP_Nome"), ""), _
                        iDataReader.Item("DFLP_Pagina"), iDataReader.Item("DFLP_Default"), ComunitaID, _
