    ...
    Public Overrides Sub BindDati()
        ...
        Select Case Me.ViewModeType
            Case EpViewModeType.Manage
                initPageManage()
            Case EpViewModeType.View
                RedirectToUrl(RootObjectMoocs.EduPathList(CurrentCommunityID, EpViewModeType.View, PreloadIsMooc))

                'If ServiceEP.isPlayModePath(CurrentPathId) Then
                '    RedirectToUrl(RootObjectMoocs.PathView(CurrentPathId, CurrentCommunityID, EpViewModeType.View, True))
                'End If
                'initPageView()
            Case Else
...
#End Region
    ...
    Private Sub InitPlayer()
        ...
        Else
            ...
            'Me.IMGhelp.ImageUrl = RootObjectMoocs.ImgHelp(Me.BaseUrl)
    ...
    Private Sub initPageView()
        ...
        Else
            ...
            If ServiceStat.InitPathBrowsed(pathId, Me.CurrentUserId, Me.CurrentUserId, OLDpageUtility.ClientIPadress, OLDpageUtility.ProxyIPadress) Then 'init path statististc
                ...
                Me.HYPlistEduPath.NavigateUrl = Me.BaseUrl & RootObjectMoocs.EduPathList(Me.CurrentCommunityID, Me.ViewModeType, dtoEP.IsMooc)
        ...
        If Not IsNothing(dtoEP) Then
            ...
            If dtoEP.Units.Count > 0 Then
                ...
                If dtoEP.PermissionEP.Create Then
                    ...
                    HYPnewUnit.NavigateUrl = Me.BaseUrl & RootObjectMoocs.NewUnit(Me.CurrentCommunityID, Me.CurrentPathId, IsMoocPath) 'path= al redirectToURL nell'else
                    Me.HYPnewUnitNote.Visible = dtoEP.PermissionEP.Create
                    Me.HYPnewUnitNote.NavigateUrl = Me.BaseUrl & RootObjectMoocs.AddTextItem(Me.CurrentPathId, ItemType.Unit, CurrentCommunityID, IsMoocPath, IsInReadOnlyMode)
                End If

                Me.HYPvalidateByAct.Visible = True
                Me.HYPvalidateByAct.NavigateUrl = Me.BaseUrl & RootObjectMoocs.Validate(CurrentPathId, CurrentCommunityID, ItemType.Activity, IsMoocPath, IsInReadOnlyMode)
            ...
            ElseIf (dtoEP.PermissionEP.Create) Then
                Me.RedirectToUrl(RootObjectMoocs.NewUnit(Me.CurrentCommunityID, Me.CurrentPathId, IsMoocPath)) 'path= a HYPnewUnit.NavigateUrl
            Else
                RedirectToUrl(RootObjectMoocs.EduPathList(CurrentCommunityID, EpViewModeType.View, dtoEP.IsMooc))
                'initPageView()
            End If
        Else
    ...
    Private Sub ShowError(ByVal errorType As EpError)
        ...
        HYPerror.NavigateUrl = Me.BaseUrl & RootObjectMoocs.EduPathList(Me.CurrentCommunityID, Me.ViewModeType, IsMoocPath)
...
#Region "Repeater Item Data Bound"
    ...
    Private Sub MoveDownAct(ByRef actId)
        If ServiceEP.MoveActivityDisplayOrderAfter(actId, Me.CurrentUserId, Me.IdCommunityRole, IsEditablePath, OLDpageUtility.ClientIPadress, OLDpageUtility.ProxyIPadress) Then
            RedirectToUrl(RootObjectMoocs.PathView(Me.CurrentPathId, Me.CurrentCommunityID, EpViewModeType.Manage, False, IsMoocPath, IsInReadOnlyMode))
        Else
            ShowError(EpError.Generic)
        End If
    End Sub
            ...
            Select Case e.CommandName
                Case "mandatory"
                    If ServiceEP.UpdateActivityMandatoryStatus(e.CommandArgument, Me.CurrentUserId, OLDpageUtility.ClientIPadress, OLDpageUtility.ProxyIPadress) Then
                        Me.PageUtility.RedirectToUrl(RootObjectMoocs.PathView(Me.CurrentPathId, Me.CurrentCommunityID, Me.ViewModeType, False, IsMoocPath, IsInReadOnlyMode))
                    Else
                        ShowError(EpError.Generic)
                    End If
                Case "visibility"
    ...
    Private Sub SetImgBlocked(ByRef oImg As System.Web.UI.WebControls.Image, ByVal isBlocked As Boolean)
        oImg.Visible = True
        oImg.ImageUrl = RootObjectMoocs.ImgItemBlocked(Me.BaseUrl, Not isBlocked)
    ...
    Private Sub SetBlockedUnitBtn(ByRef e As System.Web.UI.WebControls.RepeaterItemEventArgs, ByRef dtoItem As dtoUnit)
        ...
        If Not ServiceEP.CheckStatus(dtoItem.Status, Status.Locked) Then
            oDlkb.Text = String.Format(oDlkb.Text, RootObjectMoocs.ImgBtnBlocked_On(Me.BaseUrl), oDlkb.ToolTip)
        Else
            oDlkb.Text = String.Format(oDlkb.Text, RootObjectMoocs.ImgBtnBlocked_Off(Me.BaseUrl), oDlkb.ToolTip)
        End If
    ...
    Private Sub setUpdateUnitBtn(ByRef e As System.Web.UI.WebControls.RepeaterItemEventArgs, ByRef dtoItem As dtoUnit)
        ...
        If ServiceEP.CheckStatus(dtoItem.Status, Status.Text) Then
            oHyp.NavigateUrl = Me.BaseUrl & RootObjectMoocs.UpdateTextItem(dtoItem.Id, Me.CurrentPathId, ItemType.Unit, CurrentCommunityID, IsMoocPath, IsInReadOnlyMode)
        Else
            oHyp.NavigateUrl = Me.BaseUrl & RootObjectMoocs.UpdateUnit(Me.CurrentCommunityID, dtoItem.Id, Me.CurrentPathId, IsMoocPath, IsInReadOnlyMode)
        ...
        Select Case Me.ViewModeType
            Case EpViewModeType.Manage
                ...
                If dtoItem.PermissionEP.Update Then
                    ...
                    '    oLkb.Text = String.Format(oLkb.Text, RootObjectMoocs.ImgAddRule(Me.BaseUrl), oLkb.ToolTip)
                    '    oLkb.Visible = True
                    'Else
                    '    hideControl(oLkb)
                    'End If
                    ...
                    If Not isAutoEp AndAlso IsEditablePath Then
                        ...
                        If ServiceEP.CheckStatus(dtoItem.Status, Status.Mandatory) Then
                            oDlkb.Text = String.Format(oDlkb.Text, RootObjectMoocs.ImgRemoveMandatoryMedium(Me.BaseUrl), oDlkb.ToolTip)
                        Else
                            oDlkb.Text = String.Format(oDlkb.Text, RootObjectMoocs.ImgSetMandatoryMedium(Me.BaseUrl), oDlkb.ToolTip)
                        End If
                    ...
                    ElseIf Not IsEditablePath Then
                        ...
                        If ServiceEP.CheckStatus(dtoItem.Status, Status.Mandatory) Then
                            oImg.ImageUrl = RootObjectMoocs.ImgMandatoryMedium(Me.BaseUrl)
                            oImg.ToolTip = Me.Resource.getValue("UnitMandatory")
                        Else
                            oImg.ImageUrl = RootObjectMoocs.ImgBlindMedium(Me.BaseUrl)
                        End If
                ...
                If dtoItem.PermissionEP.Create AndAlso IsEditablePath Then
                    ...
                    Me.Resource.setHyperLink(oHyp, False, True)
                    oHyp.NavigateUrl = Me.BaseUrl & RootObjectMoocs.NewActivity(Me.CurrentCommunityID, dtoItem.Id, IsMoocPath)
                    oHyp.Visible = True
                    oHyp = e.Item.FindControl("HYPnewActNote")
                    Me.Resource.setHyperLink(oHyp, False, True)
                    oHyp.NavigateUrl = Me.BaseUrl & RootObjectMoocs.AddTextItem(dtoItem.Id, ItemType.Activity, CurrentCommunityID, IsMoocPath, IsInReadOnlyMode)
                    oHyp.Visible = True
                End If
    ...
    Private Sub RPunit_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles RPunit.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            If ServiceEP.CheckStatus(dtoItem.Status, Status.Text) Then
                Dim oImg As System.Web.UI.WebControls.Image
                oImg = e.Item.FindControl("IMGmandatory")
                oImg.ImageUrl = RootObjectMoocs.ImgTextNoteItem(Me.BaseUrl)
    ...
    Private Sub setUpdateActivityBtn(ByRef e As System.Web.UI.WebControls.RepeaterItemEventArgs, ByRef dtoItem As dtoActivity)
        ...
        Me.Resource.setHyperLink(oHyp, False, True)
        If ServiceEP.CheckStatus(dtoItem.Status, Status.Text) Then
            oHyp.NavigateUrl = Me.BaseUrl & RootObjectMoocs.UpdateTextItem(dtoItem.Id, _unitId, ItemType.Activity, CurrentCommunityID, IsMoocPath, IsInReadOnlyMode)
        Else
            oHyp.NavigateUrl = Me.BaseUrl & RootObjectMoocs.UpdateActivity(Me.CurrentCommunityID, dtoItem.Id, Me.TempUnitId, IsMoocPath, IsInReadOnlyMode)
        End If
        oHyp.Visible = True
    End Sub
            ...
            Case EpViewModeType.Manage
                ...
                If dtoItem.PermissionEP.Update Then
                    ...
                    If IsEditablePath Then
                        ...
                        oDlkb.Visible = True
                        If ServiceEP.CheckStatus(dtoItem.Status, Status.Mandatory) Then
                            oDlkb.Text = String.Format(oDlkb.Text, RootObjectMoocs.ImgRemoveMandatorySmall(Me.BaseUrl), oDlkb.ToolTip)
                        Else
                            oDlkb.Text = String.Format(oDlkb.Text, RootObjectMoocs.ImgSetMandatorySmall(Me.BaseUrl), oDlkb.ToolTip)
                        End If
                ...
                If Not IsEditablePath Then
                    ...
                    oImg.Visible = True
                    If ServiceEP.CheckStatus(dtoItem.Status, Status.Mandatory) Then
                        oImg.ImageUrl = RootObjectMoocs.ImgMandatorySmall(Me.BaseUrl)
                        oImg.ToolTip = Me.Resource.getValue("ActivityMandatory")
                    Else
                        oImg.ImageUrl = RootObjectMoocs.ImgBlindSmall(Me.BaseUrl)
                    End If
    ...
    Private Sub SetImgStatusSmall(ByRef oImg As System.Web.UI.WebControls.Image, ByRef status As StatusStatistic)
        ...
        Select Case status
            Case StatusStatistic.CompletedPassed
                oImg.ImageUrl = RootObjectMoocs.ImgStatusGreenSmall(Me.BaseUrl)
                tooltip = Resource.getValue("StatusStatistic." & StatusStatistic.CompletedPassed.ToString)
            Case StatusStatistic.Browsed
                oImg.ImageUrl = RootObjectMoocs.ImgStatusGreySmall(Me.BaseUrl)
                tooltip = Resource.getValue("StatusStatistic." & StatusStatistic.Browsed.ToString)
            Case StatusStatistic.None
                oImg.ImageUrl = RootObjectMoocs.ImgStatusGreySmall(Me.BaseUrl)
                tooltip = Resource.getValue("StatusStatistic." & StatusStatistic.Browsed.ToString)
            Case Else
                oImg.ImageUrl = RootObjectMoocs.ImgStatusYellowSmall(Me.BaseUrl)
                tooltip = Resource.getValue("StatusStatistic." & StatusStatistic.Started.ToString)
        End Select
        oImg.AlternateText = tooltip
        oImg.ToolTip = tooltip
...
#Region "GESTIONE DIALOG ATTUALMENTE ELIMINATE... IN CASO DI MODIFICA GESTIVANO SE AGGIORNARE LE STATISTICHE O PERSONALIZZARE GLI ASSEGNAMENTI DI CHI HA GIA' INIZIATO IL PERCORSO"
    ...
    '            Me.PageUtility.RedirectToUrl(RootObjectMoocs.PathView(Me.CurrentPathId, Me.CurrentCommunityID, Me.ViewModeType))
    '        Else
...
#End Region
    ...
    Private Sub GotoPathEditing(idPath As Long, status As PathAvailability)
        ...
        If (idPath > 0) Then
            ...
            End If
            PageUtility.RedirectToUrl(RootObjectMoocs.PathView(idPath, CurrentCommunityID, ViewModeType, ServiceEP.CheckEpType(ServiceEP.GetPathType(idPath), EPType.PlayMode), IsMoocPath, IsInReadOnlyMode))
        End If
    End Sub
    Private Sub BTNreadonlyOption_Click(sender As Object, e As EventArgs) Handles BTNreadonlyOption.Click
        Dim idPath As Long = CurrentPathId
        DVconfirmEditing.Visible = False
        PageUtility.RedirectToUrl(RootObjectMoocs.PathView(idPath, CurrentCommunityID, ViewModeType, ServiceEP.CheckEpType(ServiceEP.GetPathType(idPath), EPType.PlayMode), IsMoocPath, True))
    End Sub
