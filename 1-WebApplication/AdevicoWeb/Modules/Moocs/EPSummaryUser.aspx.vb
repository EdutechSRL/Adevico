...
Public Class EPSummaryUserMooc
    Inherits EPlitePageBaseMoocs
...
#Region "Implements"
    ...
    Private Sub DisplaySessionTimeout() Implements IViewSummaryUser.DisplaySessionTimeout
        ...
        dto.DestinationUrl = lm.Comol.Modules.EduPath.BusinessLogic.RootObjectMoocs.SummaryUser(PreloadFromSummary, PreloadIdUser, PreloadIdCommunity)
    ...
    Private Sub LoadData(ByVal idUser As Integer, person As litePerson) Implements lm.Comol.Modules.EduPath.Presentation.IViewSummaryUser.LoadData
        ...
        End If
        HYPback.NavigateUrl = BaseUrl & RootObjectMoocs.SummaryIndex(FromSummary, SummaryIdCommunity)
...
#Region "Repeaters"
    Private Sub RPTpaths_ItemDataBound(sender As Object, e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles RPTpaths.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            hyp.NavigateUrl = BaseUrl & RootObjectMoocs.ViewFullPlay(dto.IdPath, dto.IdCommunity, PreloadIsMooc)
            hyp.Visible = dto.CanPlay

            hyp = e.Item.FindControl("HYPstats")
            If dto.CanManage Then
                hyp.NavigateUrl = BaseUrl & RootObjectMoocs.UserStatisticsManage(dto.IdPath, dto.IdCommunity, dto.IdPerson, ItemType.Path, 0, DateTime.Now, False, SummaryType.User, SummaryIdCommunity, FromSummary, dto.IsMooc)
            Else
                hyp.NavigateUrl = BaseUrl & RootObjectMoocs.UserStatisticsView(dto.IdPath, dto.IdCommunity, DateTime.Now, False, SummaryType.User, SummaryIdCommunity, FromSummary, dto.IsMooc)
            End If
            hyp.Visible = dto.CanStat

            hyp = e.Item.FindControl("HYPedit")
            hyp.NavigateUrl = BaseUrl & RootObjectMoocs.PathView(dto.IdPath, dto.IdCommunity, EpViewModeType.Manage, False, PreloadIsMooc)
            hyp.Visible = dto.CanManage

            hyp = e.Item.FindControl("HYPsettings")
            hyp.NavigateUrl = BaseUrl & RootObjectMoocs.PathManagement(dto.IdCommunity, dto.IdPath, "-2", dto.PathType, dto.IsMooc)
            hyp.Visible = dto.CanManage

            hyp = e.Item.FindControl("HYPcertificates")
            Me.Resource.setHyperLink(hyp, False, True)
            hyp.Text = ""
            hyp.NavigateUrl = Me.BaseUrl + RootObjectMoocs.EPCertificationUser(dto.IdCommunity, dto.IdPath, dto.IdPerson, dto.IsMooc)
    ...
    Private Sub RPTcommunities_ItemDataBound(sender As Object, e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles RPTcommunities.ItemDataBound
        If (e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem) Then
            ...
            If p.Administration Then
                hyp.NavigateUrl = Me.BaseUrl & RootObjectMoocs.EduPathList(idcomm, EpViewModeType.Manage, PreloadIsMooc)
            Else
                hyp.NavigateUrl = Me.BaseUrl & RootObjectMoocs.EduPathList(idcomm, EpViewModeType.View, PreloadIsMooc)
            End If
