...
Public Class UnitManagementMooc
        ...
        Get
            If IsSessioneScaduta(False) Then
                If CurrentUnitID > 0 Then
                    RedirectOnSessionTimeOut(RootObjectMoocs.UpdateUnit(Me.CurrentCommunityID, CurrentUnitID, PathID, PreloadIsMooc, PreloadIsFromReadOnly), CurrentCommunityID)
                Else
                    RedirectOnSessionTimeOut(RootObjectMoocs.NewUnit(Me.CurrentCommunityID, PathID, PreloadIsMooc), CurrentCommunityID)
                End If
            End If
            Return False
        End Get
...
#Region "Button"
    ...
    Public Sub BTNnext_Click(ByVal sender As Object, ByVal e As EventArgs)
        Select Case Me.CurrentStep
            Case StepUnitManagement.NewUnit
                ...
                End If
                If PersistData() Then
                    RedirectToUrl(RootObjectMoocs.PathView(PathID, CurrentCommunityID, EpViewModeType.Manage, ServiceEP.isPlayModePath(PathID), IsMoocPath, False))
                Else
                    Me.ShowError(EpError.Generic)
                End If
            Case StepUnitManagement.Detail
                ...
                Else
                    Me.ShowError(EpError.Generic)
                End If
                'Me.PageUtility.RedirectToUrl(RootObjectMoocs.UnitManagement(currentUnit.Id, Me.PathID, Me.CurrentCommunityID, StepUnitManagement.SelectPermission))
            Case StepUnitManagement.SelectPermission
                Me.GetSelectedPermission()
                Me.PageUtility.RedirectToUrl(RootObjectMoocs.UnitManagement(currentUnit.Id, Me.PathID, Me.CurrentCommunityID, StepUnitManagement.Detail, IsMoocPath, PreloadIsFromReadOnly))
            Case StepUnitManagement.SelectPerson
                Me.GetSelectedPerson()
                Me.PageUtility.RedirectToUrl(RootObjectMoocs.UnitManagement(currentUnit.Id, Me.PathID, Me.CurrentCommunityID, StepUnitManagement.SelectPermission, IsMoocPath, PreloadIsFromReadOnly))
            Case StepUnitManagement.Update
                Me.UpdateDetail()
                If PersistData() Then
                    RedirectToUrl(RootObjectMoocs.PathView(PathID, CurrentCommunityID, EpViewModeType.Manage, ServiceEP.isPlayModePath(PathID), IsMoocPath, PreloadIsFromReadOnly))
    ...
    Public Sub BTNcancel_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If (From units In currentUnit.ParentPath.UnitList Where (Not ServiceEP.CheckStatus(units.Status, Status.Draft) And units.Deleted = 0) Select units).Count = 0 Then
            PageUtility.RedirectToUrl(RootObjectMoocs.EduPathList(CurrentCommunityID, EpViewModeType.Manage, IsMoocPath))
        Else
    ...
    Private Sub BTNselectPerson_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BTNselectPerson.Click
        Me.GetSelectedPermission()
        PageUtility.RedirectToUrl(RootObjectMoocs.UnitManagement(Me.currentUnit.Id, Me.PathID, Me.CurrentCommunityID, StepUnitManagement.SelectPerson, IsMoocPath, PreloadIsFromReadOnly))
    End Sub
#End Region
...
#End Region
    ...
    Private Sub BTNerror_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BTNerror.Click
        Me.ClearSession()
        Me.PageUtility.RedirectToUrl(RootObjectMoocs.EduPathList(Me.CurrentCommunityID, EpViewModeType.Manage, IsMoocPath))
    End Sub
