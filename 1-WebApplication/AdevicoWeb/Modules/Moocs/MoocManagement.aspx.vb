...
#Region " Base"
            ...
            Select Case Me.CurrentStep
                Case StepPathManagement.NewPath
                    If Me.CurrentCommunityID <= 0 Then
                        Me.PageUtility.RedirectToUrl(RootObjectMoocs.PathManagementSelectCommunity())
    ...
    Public Overrides Sub BindNoPermessi()
        If CurrentStep <> CInt(StepPathManagement.NewPath) Then
            If Not ServiceEP.IsMooc(CurrentPathID) Then
                PageUtility.RedirectToUrl(RootObject.EduPathList(CurrentCommunityID, EpViewModeType.Manage, False))
            End If
        End If
        ...
        Get
            If IsSessioneScaduta(False) Then
                If CurrentPathID > 0 Then
                    RedirectOnSessionTimeOut(RootObjectMoocs.PathManagement(CurrentCommunityID, CurrentPathID, CInt(StepPathManagement.Detail).ToString, PathType, True), CurrentCommunityID)
                Else
                    RedirectOnSessionTimeOut(RootObjectMoocs.AddPath(CurrentCommunityID, EpType.TimeAuto, True), CurrentCommunityID)
                End If
            End If
            Return False
        End Get
...
#End Region
    ...
    Private Sub InitDetail()
        ...
        If (currentPath.IsMooc) Then
            DIVmoocType.Visible = True
            Me.RBLmoocType.SelectedValue = CType(currentPath.MoocType, Integer)
        Else
            Me.RBLmoocType.SelectedValue = 0
            Me.DIVmoocType.Visible = False
        End If
    ...
    Private Function UpdateDetail() As Boolean
        ...
        If (currentPath.IsMooc) Then
            Try
                currentPath.MoocType = CType(RBLmoocType.SelectedValue, Integer)
            Catch ex As Exception
                currentPath.MoocType = lm.Comol.Modules.EduPath.Domain.MoocType.Cockade
            End Try

        Else
            currentPath.MoocType = lm.Comol.Modules.EduPath.Domain.MoocType.None
        End If
...
#Region "Button"
    ...
    Public Sub BTNnext_Click(ByVal sender As Object, ByVal e As EventArgs)
        Select Case Me.CurrentStep
            Case StepPathManagement.NewPath
                ...
                If Me.UpdateDetail() Then
                    If Not riseAlert Then
                        If PersistData() Then
                            RedirectToUrl((RootObjectMoocs.PathManagement(CurrentCommunityID, currentPath.Id, StepPathManagement.SelectPermission, EpType, currentPath.IsMooc)))
                        Else
                            Me.ShowError(EpError.Generic)
                        End If
                    End If
            ...
            Case StepPathManagement.SelectPermission
                ...
                RedirectToUrl(RootObjectMoocs.PathManagement(CurrentCommunityID, currentPath.Id, StepPathManagement.SummaryPermission, EpType, currentPath.IsMooc))
            Case StepPathManagement.SelectPerson
            ...
            Case StepPathManagement.SummaryPermission
                ...
                End If
                If PersistData() Then
                    RedirectToUrl(RootObjectMoocs.EduPathList(Me.CurrentCommunityID, EpViewModeType.Manage, currentPath.IsMooc) & "#" & CurrentPathID)
                End If
            ...
            Case StepPathManagement.SelectCommunity
                ...
                Me.PageUtility.RedirectToUrl(RootObjectMoocs.AddPath(CurrentCommunityID, EpType, PreloadIsMooc))
        End Select
    End Sub
    ...
    Public Sub BTNprevious_Click(ByVal sender As Object, ByVal e As EventArgs)
        Select Case Me.CurrentStep
            ...
            Case StepPathManagement.SelectPermission
                Me.GetSelectedPermission()
                Me.PageUtility.RedirectToUrl(RootObjectMoocs.PathManagement(CurrentCommunityID, currentPath.Id, StepPathManagement.Detail, EpType, currentPath.IsMooc))
            Case StepPathManagement.SelectPerson
                Me.GetSelectedPerson()
                Me.PageUtility.RedirectToUrl(RootObjectMoocs.PathManagement(CurrentCommunityID, currentPath.Id, StepPathManagement.SelectPermission, EpType, currentPath.IsMooc))
            Case StepPathManagement.SummaryPermission
                Me.PageUtility.RedirectToUrl(RootObjectMoocs.PathManagement(CurrentCommunityID, currentPath.Id, StepPathManagement.SelectPermission, EpType, currentPath.IsMooc))
        End Select
    End Sub
    Public Sub BTNcancel_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        End If
        ClearSession()
        PageUtility.RedirectToUrl(RootObjectMoocs.EduPathList(Me.CurrentCommunityID, EpViewModeType.Manage, currentPath.IsMooc))
    End Sub
    Private Sub BTNselectPerson_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BTNselectPerson.Click
        Me.GetSelectedPermission()
        Me.PageUtility.RedirectToUrl(RootObjectMoocs.PathManagement(CurrentCommunityID, currentPath.Id, StepPathManagement.SelectPerson, EpType, currentPath.IsMooc))
    End Sub
#End Region
...
#End Region
    ...
    Private Sub BTNerror_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BTNerror.Click
        Me.ClearSession()
        Me.PageUtility.RedirectToUrl(RootObjectMoocs.EduPathList(Me.CurrentCommunityID, EpViewModeType.Manage, currentPath.IsMooc))
    End Sub
