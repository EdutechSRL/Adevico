    ...
    Public Overrides ReadOnly Property VerifyAuthentication As Boolean
        Get
            If IsSessioneScaduta(False) Then
                RedirectOnSessionTimeOut(RootObjectMoocs.ViewFullPlay(CurrentPathId, CurrentCommunityID, True), CurrentCommunityID)
            End If
            Return False
        End Get
    End Property
...
#Region "Inherits"
    Public Overrides Sub BindDati()
        Dim isMooc As Boolean = GetIsMoocPath(CurrentPathId)
        If Not ServiceEP.isPlayModePath(CurrentPathId) Then
            If isMooc Then
                RedirectToUrl(RootObjectMoocs.PathView(CurrentPathId, CurrentCommunityID, EpViewModeType.View, True, isMooc))
            Else
                RedirectToUrl(RootObject.PathView(CurrentPathId, CurrentCommunityID, EpViewModeType.View, True, isMooc))
            End If
        ElseIf isMooc Then
            ...
            initPageView()
        Else
            RedirectToUrl(RootObject.ViewFullPlay(CurrentPathId, CurrentCommunityID, True))
        End If
    End Sub
...
#End Region
    ...
    Private Sub initPageView()
        ...
        Else
            If ServiceStat.InitPathBrowsed(idPath, Me.CurrentUserId, Me.CurrentUserId, OLDpageUtility.ClientIPadress, OLDpageUtility.ProxyIPadress) Then
                ...
                HYPstat.NavigateUrl = Me.BaseUrl & RootObjectMoocs.UserStatisticsView(idPath, Me.CurrentCommunityID, DateTime.Now, False, dtoEP.IsMooc)

                InitPlayer()
                If ServiceEP.GetPathCount_ViewMode(CurrentUserId, IdCommunityRole, CurrentCommunityID) > 2 Then
                    Me.HYPlistEduPath.NavigateUrl = Me.BaseUrl & RootObjectMoocs.EduPathList(Me.CurrentCommunityID, EpViewModeType.View, dtoEP.IsMooc)
                Else
                ...
                If VisibleActivity Is Nothing Then
                    ...
                    Dim lockActId As IList(Of Int64) = (From u In dtoEP.Units From a In u.Activities Where a.Status.HasFlag(Status.Locked) Select a.Id).ToList()
                    ...
                    If result IsNot Nothing Then
                        If Not IsNothing(lockActId) AndAlso lockActId.Any() Then
                            VisibleActivity = (From item In result
                                               Where Not lockActId.Contains(item.Key.Id)
                                               Select item.Key.Id).ToList()
                        Else
                ...
                If VisibleUnit Is Nothing Then
                    ...
                    If result IsNot Nothing Then
                        ...
                        'Nasconde unità 
                        'vuote: dUnit.Activities.Any(Function(a As dtoActivity) VisibleActivity.Contains(a.Id)) AndAlso _
                        'Dim byPass As Boolean = False

                        For Each dUnit As dtoUnit In dtoEP.Units
                            If Not dUnit.Status.HasFlag(Status.Locked) AndAlso unitsId.Contains(dUnit.Id) Then
                        ...
                        'Mostra unità vuote:
                        'VisibleUnit = (From item In result Where Select item.Key.Id).ToList()
                    End If
    ...
    Private Sub ShowError(ByVal ErrorType As EpError)
        ...
        If GetIsMoocPath(CurrentPathId) Then
            Me.HYPerror.NavigateUrl = Me.BaseUrl & RootObjectMoocs.EduPathList(Me.CurrentCommunityID, EpViewModeType.View, True)
        Else
            Me.HYPerror.NavigateUrl = Me.BaseUrl & RootObject.EduPathList(Me.CurrentCommunityID, EpViewModeType.View, False)
        End If
