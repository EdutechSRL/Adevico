<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/AjaxPortal.Master" CodeBehind="MoocPlay.aspx.vb" Inherits="Comunita_OnLine.MoocPlay" %>
<%@ Register TagPrefix="CTRL" TagName="ProgressBar" Src="~/Modules/EduPath/UC/UC_ProgressBar.ascx" %>
<%@ Register TagPrefix="COL" Assembly="Comunita_OnLine" Namespace="Comunita_OnLine.MyUC" %>
<%@ Register Src="~/Modules/EduPath/UC/UC_HelpStatus.ascx" TagName="helpStatus" TagPrefix="CTRL" %>
<%@ Register Src="~/Modules/EduPath/UC/UC_TextAction.ascx" TagName="TextAction" TagPrefix="CTRL" %>
<%@ Register Src="~/Modules/EduPath/UC/UC_CertificationAction.ascx" TagName="CertificationAction" TagPrefix="CTRL" %>
...
<!-- START HEADER FULLPLAY-->
    ...
    <link href="../../Graphics/Modules/Edupath/css/moocs.css?v=201605041410lm" rel="Stylesheet" />
    <script type="text/javascript">
        $(function () {
        ...
        $(function () {
            $(".title.switchunity.handle").each(function () {

                var x = $(".floatbar .cloneme").clone();
                x.removeClass("cloneme");



                x.find("a").html($(this).html());

                x.data("tag", $(this));
                x.data("id", $(this).parents("li.unit").first().attr("id"));
                x.find("a").attr("href", "#" + x.data("id"));

                $(".floatbar .gobottom").before(x);
                /*$(".floatbar .gobottom").before(x.clone());
				$(".floatbar .gobottom").before(x.clone());
				$(".floatbar .gobottom").before(x.clone());*/
            });

            $(".floatbar .unit.gotop").click(function () {
                window.scrollTo(0, 0);
                return false;
            });

            $(".floatbar .unit.gobottom").click(function () {
                window.scrollTo(0, document.body.scrollHeight);
                return false;
            });

            $(".floatbar .unit").click(function () {

            });

            $(document).scroll(function () {
                //var scrollPosition = $(document).scrollTop();
                var eTop = $('.floatbar').offset().top; //get the offset top of the element
                var ref = eTop - $(window).scrollTop(); //position of the ele w.r.t window


                var uTop = $('.units').offset().top;



                //var scrollReference = $(".units").scrollTop();
                if (ref <= 0) {
                    $(".floatbar").addClass('fixed');
                } else {
                    $(".floatbar").removeClass('fixed');
                    $(".floatbar").addClass('abs');
                };
    ...
    <asp:MultiView ID="MLVeduPathView" runat="server" ActiveViewIndex="0">
        <asp:View ID="VIWplay" runat="server">
            ...
            <div class="segnaposto"> 
                ...
                <div class="floatbar">
                    <div class="unit gotop">
                        <a href="">&uarr;</a>
                    </div>
                    <div class="unit cloneme">
                        <a href="">unit</a>
                    </div>
                    <div class="unit gobottom">
                        <a href="">&darr;</a>
                    </div>
                </div>
            </div>
                                                    ...
                                                    <div class="activitycontent">
                                                        ...
                                                        <div class="internal clearfix">
                                                            <span class="left">
                                                                ...
                                                                <CTRL:QuestionnaireItem ID="CTRLquestionnaire" runat="server" Visible="false" EnableAnchor="true"/>
                                                                <CTRL:DisplayItem ID="CTRLdisplayItem" runat="server" EnableAnchor="true" DisplayExtraInfo="false" DisplayLinkedBy="false"  Visible="false"/>
                                                                <CTRL:TextAction runat="server" id="CTRLtextAction" OnRefreshContainerEvent="RefreshContainerEvent"  Visible="false"/>
