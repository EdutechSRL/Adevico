    ...
    Private Function ParseContent(idLanguage As Integer, languageCode As String, content As lm.Comol.Core.DomainModel.Languages.ItemObjectTranslation, modules As System.Collections.Generic.List(Of String), community As lm.Comol.Core.DomainModel.Community, p As Person, organizationName As String, Optional obj As lm.Comol.Core.DomainModel.ModuleObject = Nothing) As lm.Comol.Core.DomainModel.Languages.ItemObjectTranslation Implements IViewMailMessage.ParseContent
        ...
        For Each moduleCode As String In modules
            Select Case moduleCode
                ...
                '    'Messo try-catch SOLO per test!
                '    Try
                '        Dim WCService As New lm.Comol.Modules.Standard.WebConferencing.Domain.WbServiceGeneric( _
                '                EWtmpHelper.GetCurrentParameters(MyBase.SystemSettings.WebConferencingSettings),
                '                PageUtility.CurrentContext)

                '        'content = WCService.GetTemplateContentPreview(True, _
                '        '        GetFakeWebConference(p, community), _
                '        '        r.getValue("Webconferencing.BaseUrl"), _
                '        '        content, _
                '        '        r.getValue("Webconferencing.DateTimeFormat"),
                '        '        r.getValue("Webconferencing.NullDateTime"))

                '        content = WCService.GetTemplateContentPreview(True, _
                '            GetFakeWebConference(p, community), _
                '            r.getValue("Webconferencing.BaseUrl"), r.getValue("Webconferencing.BaseUrl"), _
                '            content, _
                '            r.CultureInfo.DateTimeFormat.FullDateTimePattern, _
                '            r.getValue("Webconferencing.NullDateTime"))

                '        'r.getValue("Webconferencing.NullDateTime")

                '        'DOVREBBE DIVENTARE:
                '        'DA UC_MessageEdit.ascx.vb, righe:

                '        '584:
                '        'Dim wcEmptyTranslations As New Dictionary(Of String, Dictionary(Of lm.Comol.Modules.Standard.WebConferencing.Domain.PlaceHoldersType, String))

                '        '593-594:
                '        'wcEmptyTranslations.Add(t.LanguageCode, (From p In lm.Comol.Modules.Standard.WebConferencing.Domain.TemplatePlaceHolders.GetPlaceHoldersType
                '        '   Select p).ToDictionary(Function(p) p, Function(p) r.getValue("Empty.WebConference.PlaceHoldersType." & p.ToString)))

                '        'Da lm.Comol.Modules.Standard.WebConferencing.Domain.WbService.vb, riga 1325/1329
                '        'r.getValue("Webconferencing.NullDateTime") = wcEmptyTranslations[PlaceHoldersType.RoomStartDate]
                '        'r.getValue("Webconferencing.NullDateTime") = wcEmptyTranslations[PlaceHoldersType.RoomEndDate]
...
#Region "CallForPaper/Request For Membership"
    Private Function GetFakeCallForPaper(p As litePerson, ByVal c As liteCommunity) As lm.Comol.Modules.CallForPapers.Domain.CallForPaper
        ...
        With fakeCall
            ...
            .AllowSubmissionExtension = False
            .Community = c
