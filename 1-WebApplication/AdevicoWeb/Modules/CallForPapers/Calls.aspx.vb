                ...
                ElseIf Not SubmissionOpened Then
                    ...
                    For Each submission As dtoSubmissionDisplayInfo In dto.SubmissionsInfo
                        ...
                        ElseIf submission.HasWorkingRevision Then
                            ...
                            Else
                                translations.Add(String.Format(url, BaseUrl & RootObject.ViewSubmission(CallForPaperType.CallForBids, dto.Id, submission.Id, False, CurrentView, IdCallCommunity), Me.Resource.getValue("SubmissionStatus." & CallForPaperType.CallForBids.ToString() & "." & submission.Status.ToString)))
                            End If
                        Else
                            translations.Add(String.Format(url, BaseUrl & RootObject.ViewSubmission(CallForPaperType.CallForBids, dto.Id, submission.Id, False, CurrentView, IdCallCommunity), Me.Resource.getValue("SubmissionStatus." & CallForPaperType.CallForBids.ToString() & "." & submission.Status.ToString)))
                        End If
