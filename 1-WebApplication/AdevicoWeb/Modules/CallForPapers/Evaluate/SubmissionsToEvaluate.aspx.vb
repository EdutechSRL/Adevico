...
#End Region
    ...
    Private Sub SetContainerName(callName As String, endEvaluationOn As Date?) Implements IViewSubmissionsToEvaluate.SetContainerName
        ...
        End If
    End Sub
    Private Sub SetActionUrl(action As lm.Comol.Modules.CallForPapers.Domain.CallStandardAction, url As String) Implements IViewSubmissionsToEvaluate.SetActionUrl
        Select Case action
...
#Region "Helpers"
    ...
    Private Sub InitStackedBar(item As dtoBaseEvaluatorStatistics, control As UC_StackedBar)
        InitStackedBar(item.GetEvaluationsCount(), item.GetEvaluationsCount(EvaluationStatus.Evaluated), item.GetEvaluationsCount(EvaluationStatus.Evaluating), item.GetEvaluationsCount(EvaluationStatus.None), control)
    End Sub
    Private Sub InitStackedBar(item As dtoEvaluatorCommitteeStatistic, control As UC_StackedBar)
        InitStackedBar(item.Evaluations.Count, item.GetEvaluationsCount(EvaluationStatus.Evaluated), item.GetEvaluationsCount(EvaluationStatus.Evaluating), item.GetEvaluationsCount(EvaluationStatus.None), control)
    End Sub
    Private Sub InitStackedBar(count As Long, nEvaluated As Long, nEvaluating As Long, nNotstarted As Long, control As UC_StackedBar)
...
#Region "Controls"
    ...
    Private Sub RPTevaluations_ItemDataBound(sender As Object, e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles RPTevaluations.ItemDataBound
        ...
        oHyperlink.NavigateUrl = PageUtility.BaseUrl & lm.Comol.Modules.CallForPapers.Domain.RootObject.ViewSingleEvaluation(item.Id, item.IdSubmission, IdCall, IdCallCommunity)
