    ...
    Private Sub LoadFiles(files As List(Of dtoSubmissionAttachment), _
        ...
        For Each fileType As ExportFileType In _
            ...
            If availableTypes.Contains(fileType) Then
                ...
                If IsNothing(dto) OrElse WebOnlyRender Then
                    oButton.Visible = True
                Else
                    oHyperlink.Visible = True
                    oHyperlink.NavigateUrl = lm.Comol.Core.FileRepository.Domain.RootObject.DownloadFromModule(PageUtility.ApplicationUrlBase, dto.Item.Id, 0, dto.Item.DisplayName, lm.Comol.Core.FileRepository.Domain.DisplayMode.downloadOrPlay, PageUtility.UniqueGuidSession.ToString, IdCallModule, dto.ModuleLinkId, False)
