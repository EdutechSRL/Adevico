...
<%@ Register TagPrefix="CTRL" TagName="DisplayGeneric" Src="~/Modules/Common/UC/UC_ExternalModuleDisplayAction.ascx" %>
...
<!-- START HEADER FULLPLAY-->
	...
	<style type="text/css">
		...
		/* Span con il codice e nome della lingua (se impostato)*/
		span.lang
		{
		...
		/* Contenitore delle sotto-attività */
		div.subactivitycontent *
		{
			margin: 0px !important;
			vertical-align:top !important;
		}

		/* Tasto attività testuale "fatto" */
		div.subactivitycontent span.objectRender a.taskDone
		{
			float:right;
		}
	</style>
	...
	$(document).ready(function(){
		var HYD_selected_language = $("#HYD_selected_language");
		var val_selected_language = HYD_selected_language.val();
		var jQ_main_language_selector = $("#main_language_selector");
		var distinctOptions = [];
		if($("select.pf_language_selector option").size() <= 0){
			jQ_main_language_selector.hide();
			return;
		}
			
		$("select.pf_language_selector option").each(function(index, el){
			var jEl = $(el);
			var val = jEl.attr("value");
			var valText = jEl.html();
			if(val && val != "" && distinctOptions.filter(function(a){ return a.value == val; }).length == 0)
			{
				distinctOptions.push({ value : val, text : valText });
			}			
		});
		
		for	(var i = 0; i < distinctOptions.length; i++)
		{
			var thisEl = distinctOptions[i];
			jQ_main_language_selector.append(
				"<option " + ((thisEl.value == val_selected_language) ? "selected " : "") + 
				"value=\"" + thisEl.value + "\">" + thisEl.text + "</option>"
			);
			
		}	
		$("select.pf_language_selector option[value='" + val_selected_language + "']").each(function(){
			$(this).parents("select.pf_language_selector:first").val(val_selected_language);
		});
		
		
		// eventi onchange selects
		jQ_main_language_selector.on("change", function(){
			HYD_selected_language.val(jQ_main_language_selector.val());
			$("select.pf_language_selector").val("");
			$("select.pf_language_selector option[value='" + jQ_main_language_selector.val() + "']").each(function(){
				$(this).parents("select.pf_language_selector:first").val(jQ_main_language_selector.val());
			});
			$("select.pf_language_selector").change();
		});
		$("select.pf_language_selector").on("change", function(){
			var thisEl = $(this);
			if(thisEl.val() && thisEl.val() != ""){
			  thisEl.parents(".activitycontent").find(".lang").parents("li.subactivity").hide();
			  thisEl.parents(".activitycontent").find(".lang.lang_" + thisEl.val()).parents("li.subactivity").show();
			}else{
			  thisEl.parents(".activitycontent").find(".lang").parents("li.subactivity").show();        
			}
			thisEl.parents(".activitycontent").find(".lang.lang_Multi").parents("li.subactivity").show();
		});
		$("select.pf_language_selector").change();
	});
</script>
...
<asp:Content ID="Content4" ContentPlaceHolderID="CPHservice" runat="server">
	<asp:MultiView ID="MLVeduPathView" runat="server" ActiveViewIndex="0">
		<asp:View ID="VIWplay" runat="server">
            ...
            <asp:PlaceHolder ID="PHlangSelector" runat="server">
			    <div class="main_language_selector_container">
				    <select name="main_language_selector" id="main_language_selector" class="pf_main_language_selector">
					    <option value=""><asp:Literal ID="LITlangAll" runat="server">Tutte</asp:Literal></option>
				    </select>
				    <asp:HiddenField ID="HYD_selected_language" runat="server" ClientIDMode="Static" Value=""/>
			    </div>
            </asp:PlaceHolder>
			<div class="tree">                
								...
								<div class="innerwrapper">
									...
									<div class="unitdetails">
										...
										<asp:Repeater ID="RPTUnitRules" runat="server">
										    <HeaderTemplate>
                                                <div class="unitrules collapsable">
                                            </HeaderTemplate>
										    <ItemTemplate>
											    <div class="divRule">                                                        
												...
												<asp:View ID="VIWMultiActivity" runat="server">                                                                                            
													...
													<div class="activitycontent" id="DIVGenericActivity" runat="server">
														...
														<div class="internal clearfix">
															...
															<span class="right">
																<asp:DropDownList ID="DDLlang" runat="server" CssClass="pf_language_selector">
															...
															<ItemTemplate>
																<li class="subactivity<%#GetItemStatus(Container.DataItem)%>" title="<%#GetItemStatusTitle(Container.DataItem)%>">
																	<div class="subactivitycontent">
																		...
																		<span class="statuslight" runat="server" id="ImgStatus">&nbsp;</span>
																		<asp:Literal ID="LTlanaguage" runat="server">
																			<span class="lang lang_{0}" title="{1}">{0}</span>
																		</asp:Literal>
																		<CTRL:QuestionnaireItem ID="CTRLquestionnaire" runat="server" EnableAnchor="false"  Visible="false"/>
                                                                        <CTRL:DisplayGeneric ID="CTRLdisplayGeneric" runat="server" visible="false"/>
													...
													<div class="activitycontent">
														...
														<div class="internal clearfix">
															<span class="left">
																...
																<CTRL:DisplayItem ID="CTRLdisplayItem" runat="server" EnableAnchor="true" DisplayExtraInfo="false" DisplayLinkedBy="false"  Visible="false"/>
																<CTRL:DisplayGeneric ID="CTRLdisplayGeneric" runat="server" />
																<CTRL:TextAction runat="server" id="CTRLtextAction" OnRefreshContainerEvent="RefreshContainerEvent"  Visible="false"/>
    ...
    <asp:Literal ID="LTtest_Item" runat="server"></asp:Literal>
</asp:Content>